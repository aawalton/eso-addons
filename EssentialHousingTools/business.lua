...
function EHT.Biz.SummonDismissAssistant( link )
	local key, collectibleId = nil, nil

	if "|H0:collectible:267|h|h" == link then
		key = "Tythis the Banker"
		collectibleId = 267
	elseif "|H0:collectible:300|h|h" == link then
		key = "Pirharri the Smuggler"
		collectibleId = 300
	elseif "|H0:collectible:301|h|h" == link then
		key = "Nuzhimeh the Merchant"
		collectibleId = 301
	elseif "|H0:collectible:6376|h|h" == link then
		key = "Ezabi the Banker"
		collectibleId = 6376
	elseif "|H0:collectible:6378|h|h" == link then
	...
	if not IsOwnerOfCurrentHouse() then
		d( "Assistants may only be summoned in one of your own homes." )
		return false
	end

	if not EHT.SavedVars.SummonAssistants then
		if not EHT.SuppressAssistantSummoningDisabledWarning then
			d( "Assistant summoning is not enabled. Enable 'Summon/Dismiss Assistants with Quickslots' under Settings > Essential Housing Tools to use this feature." )
			EHT.SuppressAssistantSummoningDisabledWarning = true
		end
...
function EHT.Biz.QueuedBuild( build, callback, buildDelay )
	...
	elseif build.TemplateName == EHT.CONST.BUILD_TEMPLATE.CONE then
		...
		local unitAngle = math.unnan( math.rad( 360 ) / circum )
		...
		repeat
			...
			if nil ~= item then
				...
				if columnIndex >= adjustedCircum then
					...
					unitAngle = math.unnan( math.rad( 360 ) / adjustedCircum )
					columnIndex = 0
					angle = 0
				end
			end
...
---[ Operations : Tab Target ]---

function EHT.Biz.TabTargetFurniture()
	EHT.TabTargetCallbackId = zo_callLater( function( id ) EHT.Biz.TabTargetFurnitureHandler( id ) end, 2 * EHT.Setup.GetEditDelay() )
end

function EHT.Biz.TabTargetFurnitureHandler( callbackId )
	if callbackId ~= EHT.TabTargetCallbackId then return end

	local selectedId = HousingEditorGetSelectedFurnitureId()
	local isReverse = IsShiftKeyDown()
	EHT.TabTargeting = true

	if nil == EHT.OriginTabTargetId or nil == EHT.AdjacentTabTargetIds then
		EHT.OriginTabTargetId = selectedId

		if nil == selectedId then
			if not HousingEditorCanSelectTargettedFurniture() then
				EHT.TabTargeting = false
				d( "Target a furniture item." )
				return
			end

			HousingEditorSelectTargettedFurniture()
			EHT.OriginTabTargetId = HousingEditorGetSelectedFurnitureId()
		else
			EHT.OriginTabTargetId = selectedId
		end

		EHT.AdjacentTabTargetIds = EHT.Housing.FindAdjacentFurniture( EHT.OriginTabTargetId, EHT.SavedVars.TabTargetRadius, false, false )

		if 1 >= #EHT.AdjacentTabTargetIds then
		...
		if nil == selectedId then return end
	end

	HousingEditorRequestModeChange( HOUSING_EDITOR_MODE_SELECTION )

	for index, id in ipairs( EHT.AdjacentTabTargetIds ) do
		if string.fromId64( id ) == string.fromId64( selectedId ) then
			local nextId = nil

			if not isReverse then
				if index < #EHT.AdjacentTabTargetIds then
					nextId = EHT.AdjacentTabTargetIds[ index + 1 ]
				else
					nextId = EHT.AdjacentTabTargetIds[ 1 ]
				end
			else
				if index > 1 then
					nextId = EHT.AdjacentTabTargetIds[ index - 1 ]
				else
					nextId = EHT.AdjacentTabTargetIds[ #EHT.AdjacentTabTargetIds ]
				end
			end

			HousingEditorSelectFurnitureById( nextId )
			break
		end
	end
end
...
function EHT.Biz.QueueQueryStatefulFurniture()
	EVENT_MANAGER:RegisterForUpdate( EHT.STATE_CHECK_ID, EHT.STATE_CHECK_INTERVAL, EHT.Biz.QueryStatefulFurniture )
end

do
	local currentQueueKey = nil

	function EHT.Biz.QueryStatefulFurniture()
		if not EHT.Housing.IsHouseZone() then
			EVENT_MANAGER:UnregisterForUpdate( EHT.STATE_CHECK_ID )
			return
		end

		--for numProcessed = 1, 5 do
		--repeat
			currentQueueKey = next( EHT.FurnitureStates, currentQueueKey )
			if not currentQueueKey then
				return
			end

			local changed, currentState, previousState = EHT.Housing.HasFurnitureStateChanged( currentQueueKey )
			if changed then
				EHT.FurnitureStates[ currentQueueKey ] = currentState
				EHT.Handlers.OnFurnitureStateChanged( currentQueueKey, currentState, previousState )
			end
		--until not currentQueueKey
		--end
	end
end
...
function EHT.Biz.ResetTargetDummies( callback )
	if nil ~= callback and "function" ~= callback then callback = nil end

	local group = EHT.Housing.GetAllTargetDummies()
	return EHT.Biz.ResetFurniture( group, callback )
end
...
function EHT.Biz.MatchTriggersByFurnitureState( furnitureId, state )
	...
	for index, trigger in pairs( triggers ) do
		...
		if idString == trigger.Condition.FurnitureId then
			if state == trigger.Condition.State or ( state ~= trigger.PreviousState and trigger.Condition.State == EHT.STATE.TOGGLE ) then
				trigger.ConditionMet = true
				if nil == results then results = { } end
				table.insert( results, trigger )
				added = true
			...
			--if trigger.Condition.Recursion or not EHT.Biz.IsTriggerQueued( trigger ) then
			-- Update last known Condition State for this trigger.
			trigger.PreviousState = state
			--end
		end
