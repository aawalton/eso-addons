...
function EHT.Housing.IdentifyTargetFurnitureAndNode( callback )
	if "function" ~= type( callback ) then
		df( "EHT.Housing.IdentifyTargetFurnitureAndNode: Argument 'callback' missing or invalid.")
		return
	end
	...
	if mode == HOUSING_EDITOR_MODE_DISABLED then
		EHT.Util.MatchUnsheathedCameraPerspectiveToCurrentPerspective()

		local frameCount = 0
		EVENT_MANAGER:RegisterForUpdate( "IdentifyTargetFurnitureAndNodeUpdate", 1, function()
			frameCount = frameCount + 1
			
			if 1 == frameCount then
				HousingEditorRequestModeChange( HOUSING_EDITOR_MODE_SELECTION )
			end

			if EHT.Housing.IsSelectionMode() then
				if frameCount >= 3 then
					EVENT_MANAGER:UnregisterForUpdate( "IdentifyTargetFurnitureAndNodeUpdate" )
					id = EHT.Housing.GetTargetFurnitureId()
	...
	elseif mode == HOUSING_EDITOR_MODE_SELECTION then
		...
		id = EHT.Housing.GetSelectedFurnitureAndNode()
	end
...
function EHT.Housing.CacheInitialFurnitureState( furnitureId )
	local key = string.fromId64( furnitureId )
	if not key then
		return false
	end

	local currentState = EHT.Housing.GetFurnitureState( furnitureId )
	EHT.FurnitureStates[ key ] = currentState
end

function EHT.Housing.CacheAllInitialFurnitureStates()
	EHT.FurnitureStates = { }
...
function EHT.Housing.HasFurnitureStateChanged( furnitureId )
	local key = string.fromId64( furnitureId )
	if not key then
		return false
	end

	local previousState = EHT.FurnitureStates[key]
	local currentState = EHT.Housing.GetFurnitureState( furnitureId )
	if currentState ~= previousState then
		EHT.FurnitureStates[key] = currentState
	end

	return previousState ~= currentState, currentState, previousState
end
...
function EHT.Housing.GetFurnitureStateByStateName( stateName, furnitureId )
	...
	--local isDoorwayCategory = EHT.Housing.IsFurnitureInCategoryOrSubcategory( furnitureId, EHT.CONST.FURNITURE_CATEGORY_ID.STRUCTURES_DOORWAYS )
	--if isDoorwayCategory then
...
function EHT.Housing.GetFurnitureState( furnitureId )
	...
	if 0 == GetPlacedHousingFurnitureNumObjectStates( furnitureId ) then return nil end
	local stateIndex = GetPlacedHousingFurnitureCurrentObjectStateIndex( furnitureId )
	if nil == stateIndex then
		return nil
	end

	if EHT.Housing.DoesFurnitureHaveIndexableStateNames( furnitureId ) then
		local stateName = GetPlacedFurniturePreviewVariationDisplayName( furnitureId, stateIndex + 1 )
		return EHT.Housing.GetFurnitureStateByStateName( stateName, furnitureId )
	end

	return DEFAULT_FURNITURE_STATES[ stateIndex + 1 ]
end
	...
	if furnitureId then
		...
		if 0 < numStates then
			if queueEntry.state ~= EHT.Housing.GetFurnitureState(furnitureId) then
				...
				--EHT.FurnitureStates[queueEntry.key] = queueEntry.state
				--df("%s: %s", self.EffectType.Name or "nil", furnitureState or "nil")
...
function EHT.Housing.ClearCachedFurnitureState(furnitureId)
	local key = string.fromId64(furnitureId)
	if key then
		EHT.FurnitureStates[key] = nil
		if EHT.FurniturePendingStates[key] then
			EHT.FurniturePendingStates[key] = nil
			for queueIndex, queueEntry in pairs(EHT.FurnitureStateQueue) do
				if queueEntry.key == key then
					table.remove(EHT.FurnitureStateQueue, queueIndex)
					break
				end
