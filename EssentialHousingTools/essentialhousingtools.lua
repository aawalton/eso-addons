...
function EHT.Setup.SetupSlashCommands()
	...
	SLASH_COMMANDS[ "/resetdummy" ] = EHT.Biz.ResetTargetDummies
	SLASH_COMMANDS[ "/resetdummies" ] = EHT.Biz.ResetTargetDummies
...
function EHT.Setup.CleanVars()
	...
	if vars.TabTargetRadius < EHT.CONST.FIND_ADJACENT_RADIUS_MIN then vars.TabTargetRadius = EHT.CONST.FIND_ADJACENT_RADIUS_MIN
	elseif vars.TabTargetRadius > EHT.CONST.FIND_ADJACENT_RADIUS_MAX then vars.TabTargetRadius = EHT.CONST.FIND_ADJACENT_RADIUS_MAX end
...
--[[
	...
	table.insert( options, {
		type = "checkbox",
		name = "Show Item Edit HUD",
		tooltip = "When toggled ON, a window will display information related to items while moving them.  This information includes the item position, orientation and, optionally, inventory data from DecoTrack.\n\n** Inventory data requires DecoTrack to be installed and active",
		getFunc = function() return EHT.SavedVars.EnableHUDItemData end,
		setFunc = function(value) EHT.SavedVars.EnableHUDItemData = value end,
	...
	table.insert( options, {
		type = "checkbox",
		name = "Include DecoTrack Data in Item Edit HUD",
		tooltip = "When toggled ON, inventory data collected from DecoTrack will be included in the window displaying information related to items while moving them.\n\n** Inventory data requires DecoTrack to be installed and active",
	...
	table.insert( options, {
		type = "slider",
		name = "Tab Target Radius (centimeters)",
		tooltip = "Tab Target selects furniture near to the originally targeted furnishing. Use this setting to control the maximum distance that other selectable furnishings can be. Measured in centimeters.",
		autoSelect = true,
		clampInput = true,
		decimals = 0,
		step = 10,
		...
		getFunc = function() return EHT.SavedVars.TabTargetRadius end,
		setFunc = function(value) EHT.SavedVars.TabTargetRadius = value end,
		default = EHT.SAVED_VARS_DEFAULTS.TabTargetRadius,
		disabled = function() return false end,
	} )
...
--[[
	...
	table.insert( options, {
		type = "checkbox",
		name = "Hide \"Identify\" Keybind",
		tooltip = "Toggle this ON to permanently hide the \"Identify\" keybind that appears while you are a guest in another player's home.\n\n" ..
			"Note that the keybind still functions, allowing you to identify any furnishing in any home, regardless of this setting.",
		key = "HideIdentifyFurnishingKeybind",
		getFunc = function() return EHT.GetSetting( "HideIdentifyFurnishingKeybind" ) end,
		setFunc = function(value)
			EHT.SavedVars.HideIdentifyFurnishingKeybind = value
			EHT.HUDFragment:UpdateVisibility()
		end,
		default = false,
		disabled = function() return false end,
	} )
...
--[[
	table.insert( options, {
		type = "checkbox",
		name = "Setup important Control keybinds automatically",
		tooltip = "|cffffffWhen toggled ON, important Controls that have no keybind will be assigned default keys when possible.\n\n" ..
			"The default keybinds are:\n" ..
			"|cffff99ALT+H|r = |c99ffffHousing Hub|r\n" ..
			"|cffff99ALT+S|r = |c99ffffSummon Storage|r\n" ..
			"|cffff99G|r = |c99ffffSelect/Deselect Items|r\n" ..
			"|cffff99F1|r = |c99ffffShow/Hide Essential Housing Tools|r\n" ..
			"|cffff99TAB|r = |c99ffffCycle through nearby items|r\n" ..
			"|cffff99SHIFT+E|r = |c99ffffEdit Item Position|r\n" ..
			"|cffff99Press & Hold O|r = |c99ffffOrganize|r",
		key = "AutoKeybindControls",
		getFunc = function() return EHT.GetSetting( "AutoKeybindControls" ) end,
		setFunc = function(value) EHT.SavedVars.AutoKeybindControls = value end,
		default = true,
		disabled = function() return false end,
		isDangerous = true,
		warning = "Changing this setting will reload your UI. Continue?",
		requiresReload = true,
	} )
]]
	table.insert( options, {
	...
	table.insert( options, {
		type = "checkbox",
		name = "Display House Information upon entry",
		tooltip = "When toggled ON, the house name and owner will be briefly displayed upon entering a house.",
		getFunc = function() return EHT.SavedVars.DisplayHouseInfoUponEntry end,
		setFunc = function(value) EHT.SavedVars.DisplayHouseInfoUponEntry = value end,
		default = EHT.SAVED_VARS_DEFAULTS.DisplayHouseInfoUponEntry,
		disabled = function() return false end,
	} )

	table.insert( options, {
		type = "checkbox",
		name = "Guest Arrival / Depature Notification",
		tooltip = "When toggled ON, notifies you in the Chat window whenever a guest enters or leaves the house you are currently in.",
		getFunc = function() return EHT.SavedVars.Doorbell end,
		setFunc = function(value) EHT.SavedVars.Doorbell = value end,
		default = EHT.SAVED_VARS_DEFAULTS.Doorbell,
		disabled = function() return false end,
	} )
...
function EHT.Setup.SetupStrings()
	...
	do
		...
		EHT.Setup.SetupKeybindStringId( layerName, "SI_BINDING_NAME_EHT_TAB_TARGET", "Tab Target", { KEY_TAB, 0, 0, 0, 0 } )
	...
	do
		...
		EHT.Setup.SetupKeybindStringId( layerName, "SI_BINDING_NAME_EHT_IDENTIFY_FURNISHING", "Identify Furnishing", { KEY_I, KEY_SHIFT, 0, 0, 0 } )
