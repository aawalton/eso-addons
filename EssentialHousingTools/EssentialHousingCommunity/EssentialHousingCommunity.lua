...
function C.DumpStats()
	...
	for _, categoryData in ipairs(categoryList) do
		df("- %s, %d homes", categoryData.name, categoryData.count)
		if "(Uncategorized)" ~= categoryData.name then
			categorizedHomes = categorizedHomes + categoryData.count
		end
	end
	df("%d total categorized homes", categorizedHomes)

	d("___________")
	d("Open Houses")
	df("- Europe, %d homes", openHouses.eu)
	df("- North America, %d homes", openHouses.na)
	df("%d total homes", openHouses.na + openHouses.eu)
end
...
function C.DumpUsers( startIndex )
	local ids, list = { }, { }
	local signatures = 0
	...
	df( "Shown %d through %d of %d total users", startIndex, endIndex, #list )
end
