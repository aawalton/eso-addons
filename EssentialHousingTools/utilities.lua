...
function EHT.Util.GetInGameTime()
	if inGameTimeOverride then
		return inGameTimeOverride.hours, inGameTimeOverride.minutes, 0
	end

	local offsetTime = GetTimeStamp() - GAMETIME_OFFSET
	local gameTime = offsetTime % REALSECONDSPERGAMEDAY
	local hours = math.floor( ( gameTime / REALSECONDSPERGAMEHOUR ) % 24 )
	local minutes = math.floor( ( gameTime / REALSECONDSPERGAMEMINUTE ) % 60 )
	local seconds = math.floor( ( gameTime / REALSECONDSPERGAMESECOND ) % 60 )
...
function EHT.Util.GetInGameTimeCommand()
	local h, m, s = EHT.Util.GetInGameTime()
	local half = "AM"
	...
	df( "Tamriel time is %.2d:%.2d:%.2d %s.", h, m, s, half )
end
...
if not writInfo and EHT.TestWritStations then
	local link = table.remove( EHT.TestWritStations, 1 )
	writInfo = EHT.Util.GetMasterWritLinkInfo( link )
end
