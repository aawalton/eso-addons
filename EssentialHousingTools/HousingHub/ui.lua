...
--[ DYNAMIC FUNCTIONS NEEDED ]--
--[[
	local effects, timestamp = self:GetHouseEffects(houseId, data.Owner)
	EHH.Effect:PublishFX(houseId, SUPPRESS_MESSAGE)
	EHH.Effect:OpenMailbox()
	EHH.Effect:CloseMailbox()
	EHH.Effect:CanShowGuestbook()
	EHH.Effect:DismissGuestbook()
  ]]

local Textures = EHH.Textures

function EHH:HideProgressIndicatorDialog()
	-- TODO
end

function EHH:UpdateProgressIndicator(message, progress)
	-- TODO
end
...
function EHH:ShowPopulationNotification(message)
	if not self.IsEHT then
		self:ShowChatMessage("|c00ccff%s|r", message)
	end
end

function EHH:OnHouseGuestsPresent(population)
	...
	local houseOwner = self:GetOwner()

	if self:IsOwner() then
		self:ShowPopulationNotification(string.format("%d guest%s %s here in your %s.", numGuests, plural, verb, houseName))
	else
		self:ShowPopulationNotification(string.format("%d other guest%s %s here in %s's %s.", numGuests, plural, verb, houseOwner, houseName))
	end
end
...
function EHH:ShowHousePopulationChanged(population)
	...
	local maxPopulation = self:GetHousePopulationCap()

	if population > previousPopulation then
		if isOwner then
			self:ShowPopulationNotification(string.format("A guest has arrived to your %s. (%d of %d max)", houseName, population, maxPopulation))
		else
			self:ShowPopulationNotification(string.format("A guest has arrived to %s's %s. (%d of %d max)", houseOwner, houseName, population, maxPopulation))
		end
	else
		if isOwner then
			self:ShowPopulationNotification(string.format("A guest has departed from your %s. (%d of %d max)", houseName, population, maxPopulation))
		else
			self:ShowPopulationNotification(string.format("A guest has departed from %s's %s. (%d of %d max)", houseOwner, houseName, population, maxPopulation))
		end
	end
...
function EHH:ShowCustomDialog(data)
	...
	if edit then
		...
		ZO_EditDefaultText_Initialize(dialog.EditBox, defaultText or "")
	else
...
function EHH:ParseHouseLinks(text)
	if self.IsEHT then
		return
	end
	...
	if tagEndIndex then
		...
		if ownerEndIndex then
			...
			if houseNameEndIndex then
				local houseName = self:Trim(string.sub(text, houseNameStartIndex, houseNameEndIndex - 1))
				local houseData = self:GetHouseByName(houseName)
				local houseId
				...
				self:ShowChatMessage(self:CreateHouseLink(ownerName, houseId))
			end
		end
	end
end
...
function EHH:CreateHouseLinkSymbol(ownerName, houseId)
	houseId = tonumber(houseId) or 0
...
function EHH:ShareHouseLink(ownerName, houseId)
	if CHAT_SYSTEM then
		local link = self:CreateHouseLinkSymbol(ownerName, houseId)
...
function EHH:ShareHubHouseLink(control)
	if "table" == type(control.Data) then
		...
		if not ownerName then
			ownerName = self.DisplayNameLower
		end
...
function EHH:OnHouseLinkClicked(houseId, owner)
	...
	local message = string.format("Visit %s|c88ffff%s|r of |c88ffff%s|r?", prep, houseName, owner)
	local function jumpFunction()
		self:JumpToHouse(houseId, owner)
	end

	self:ShowConfirmationDialog(message, jumpFunction)
end
...
do
	function EHH:SetupHousingHub()
		...
		if not ui then
			...
			do
				local logo = WINDOW_MANAGER:CreateControl(nil, win, CT_TEXTURE)
				ui.HubLogo = logo
				logo:SetTexture(Textures.HUB_LOGO)
				logo:SetTextureReleaseOption(RELEASE_TEXTURE_AT_ZERO_REFERENCES)
				-- logo:SetAnchor(TOPLEFT, nil, nil, 0, -197)
				-- logo:SetAnchor(BOTTOMRIGHT, nil, nil, 0, 197)
				logo:SetAnchor(TOPLEFT, nil, nil, 4, 4)
				logo:SetAnchor(BOTTOMRIGHT, nil, nil, -4, -56)
				logo:SetBlendMode(TEX_BLEND_MODE_COLOR_DODGE)
				logo:SetColor(1, 1, 1, 0.75)

				local beams = WINDOW_MANAGER:CreateControl(nil, win, CT_TEXTURE)
				ui.HubLogoBeams = beams
				beams:SetTexture(Textures.HUB_LOGO_CONNECTIONS)
				beams:SetTextureReleaseOption(RELEASE_TEXTURE_AT_ZERO_REFERENCES)
				-- beams:SetAnchor(TOPLEFT, nil, nil, 0, -197)
				-- beams:SetAnchor(BOTTOMRIGHT, nil, nil, 0, 197)
				beams:SetAnchor(TOPLEFT, nil, nil, 4, 4)
				beams:SetAnchor(BOTTOMRIGHT, nil, nil, -4, -56)
				beams:SetBlendMode(TEX_BLEND_MODE_COLOR_DODGE)
				beams:SetColor(1, 1, 1, 0.5)

				local ROTATION_INTERVAL = 600000
				local OFFSET_INTERVAL = 250000
				local SCALE_INTERVAL = 300000
				local GLOW_INTERVAL_1 = 3500
				local GLOW_INTERVAL_2 = 9000
				local SCALE_Y_HEIGHT = self.Defs.Controls.HubWindow.Height - 60
				local SCALE_Y_WIDTH = self.Defs.Controls.HubWindow.Width - 8
				local SCALE_Y_COEFFICIENT = SCALE_Y_WIDTH / SCALE_Y_HEIGHT
				local RAD180 = math.pi
				local RAD360 = 2 * RAD180

				local function OnUpdate()
					local ft = GetFrameTimeMilliseconds()
					local glow = 0.7 + 0.5 * math.sin(((ft % GLOW_INTERVAL_1) / GLOW_INTERVAL_1) * RAD180) + 0.3 * math.sin(((ft % GLOW_INTERVAL_2) / GLOW_INTERVAL_2) * RAD180)
					local angle = ((ft % ROTATION_INTERVAL) / ROTATION_INTERVAL) * RAD360
					local offsetAngle = ((ft % OFFSET_INTERVAL) / OFFSET_INTERVAL) * RAD360
					local offsetX = 0.5 + 0.1 * math.sin(offsetAngle)
					local offsetY = 0.5 + 0.1 * math.cos(offsetAngle)
					local scaleCoefficient = 2 + 0.75 * math.sin(((ft % SCALE_INTERVAL) / SCALE_INTERVAL) * RAD360)
					local scaleX, scaleY = scaleCoefficient, scaleCoefficient * SCALE_Y_COEFFICIENT

					ZO_ScaleAndRotateTextureCoords(logo, angle, offsetX, offsetY, scaleX, scaleY)
					ZO_ScaleAndRotateTextureCoords(beams, angle, offsetX, offsetY, scaleX, scaleY)
					beams:SetTextureSampleProcessingWeight(TEX_SAMPLE_PROCESSING_RGB, glow)
				end

				ui.HubBackdrop:SetHandler("OnEffectivelyShown", function()
					EVENT_MANAGER:RegisterForUpdate("EssentialHousingHub.HubBackdropAnimation", 1, OnUpdate)
				end)

				ui.HubBackdrop:SetHandler("OnEffectivelyHidden", function()
					EVENT_MANAGER:UnregisterForUpdate("EssentialHousingHub.HubBackdropAnimation")
				end)
			end
--[[
			ui.HubBackdropOverlays = {}
			for index = 1, 2 do
				c = WINDOW_MANAGER:CreateControl(nil, win, CT_TEXTURE)
				ui.HubBackdropOverlays[index] = c
				c:SetTexture(Textures.HUB_BACKDROP_OVERLAY)
				c:SetTextureReleaseOption(RELEASE_TEXTURE_AT_ZERO_REFERENCES)
				c:SetAddressMode(TEX_MODE_WRAP)
				c:SetAnchor(TOPLEFT)
				c:SetAnchor(BOTTOMRIGHT, nil, BOTTOM, 200, -52)
				c:SetBlendMode(TEX_BLEND_MODE_COLOR_DODGE)
				c:SetVertexColors(5, 1, 1, 1, 1)
				c:SetVertexColors(10, 0, 0, 0, 0)
			end

			c = WINDOW_MANAGER:CreateControl(nil, win, CT_TEXTURE)
			ui.HubLogo = c
			c:SetTexture(Textures.HUB_LOGO)
			c:SetTextureReleaseOption(RELEASE_TEXTURE_AT_ZERO_REFERENCES)
			c:SetAnchor(TOPLEFT)
			c:SetAnchor(BOTTOMRIGHT, nil, nil, nil, -52)
			c:SetBlendMode(TEX_BLEND_MODE_COLOR_DODGE)
			c:SetColor(1, 1, 1, 1)
			c:SetTextureCoords(-0.35, 0.77, 0.3, 0.88)

			c = WINDOW_MANAGER:CreateControl(nil, win, CT_TEXTURE)
			ui.HubLogoConnections = c
			c:SetTexture(Textures.HUB_LOGO_CONNECTIONS)
			c:SetTextureReleaseOption(RELEASE_TEXTURE_AT_ZERO_REFERENCES)
			c:SetAnchor(TOPLEFT)
			c:SetAnchor(BOTTOMRIGHT, nil, nil, nil, -52)
			c:SetBlendMode(TEX_BLEND_MODE_COLOR_DODGE)
			...
			do
				local COEFF = 0.1
				local INTERVAL = 180000
				local INTERVAL_DIV = 1 / INTERVAL
				local SCALE_INTERVAL = 20000
				local SCALE_INTERVAL_DIV = 1 / SCALE_INTERVAL
				local SAMPLE_INTERVAL = 10000
				local SAMPLE_INTERVAL_DIV = 1 / SAMPLE_INTERVAL
				local PI = math.pi
				local TWO_PI = 2 * PI
				local guiWidth, guiHeight = ui.Window:GetDimensions()
				local aspectRatio = guiWidth / guiHeight
				
				local function OnUpdate()
					local ft = GetFrameTimeMilliseconds()
					ui.HubLogoConnections:SetColor(0.407, 0.50, 0.812, 0.2 + 0.5 * (math.sin(((ft % 7000) / 7000) * PI) + math.sin(((ft % 3900) / 3900) * PI)))

					for index = 1, 2 do
						ft = ft + 15000
						local control = ui.HubBackdropOverlays[index]
						local scaleInterval = (ft % SCALE_INTERVAL) * SCALE_INTERVAL_DIV
						local scale = zo_lerp(1.5, 2, scaleInterval)
						local sampleInterval = (ft % SAMPLE_INTERVAL) * SAMPLE_INTERVAL_DIV
						local sample = math.min(0.8, 2.5 * math.sin(sampleInterval * PI))
						local interval = (ft % INTERVAL) * INTERVAL_DIV
						local angle

						if 2 == index then
							angle = -interval * TWO_PI
						else
							angle = interval * TWO_PI
						end

						self:TransformTexture(control, angle, 0.5, 0.5, scale, aspectRatio * scale)
						control:SetTextureSampleProcessingWeight(TEX_SAMPLE_PROCESSING_RGB, sample)
					end
				end

				ui.HubBackdrop:SetHandler("OnEffectivelyShown", function()
					EVENT_MANAGER:RegisterForUpdate("EssentialHousingHub.HubBackdropOverlay", 1, OnUpdate)
				end)

				ui.HubBackdrop:SetHandler("OnEffectivelyHidden", function()
					EVENT_MANAGER:UnregisterForUpdate("EssentialHousingHub.HubBackdropOverlay")
				end)
			end
]]
			...
			local function addTabButton(template, label, key, tooltip, rightAlign, addNewIcon, margin)
				...
				if tooltip then
					self:SetInfoTooltip(btn, tooltip, BOTTOM, 0, -10, TOP)
				end
			...
			local LIVE_STREAMS_TAB_TEMPLATE = "HousingHubLiveStreamsTabButton"
			...
			addTabButton(LIVE_STREAMS_TAB_TEMPLATE, "Live Streams", "Live Streams", "Live streams of ESO Housing content creators.", RIGHT_ALIGN, ADD_NEW_ICON)
--[[
			do
			...
			do
				...
				widget.Backdrop:SetDrawLayer(DL_TEXT)
...
--[[
		if updateList and not communityOpenHouseMetaData then
			-- Attempt to look up open house metadata.
			local displayNameLower = self.DisplayNameLower
			for _, record in ipairs(updateList) do
				if not record.HouseCategory then
					local owner, houseId = record.Owner or displayNameLower, record.HouseId or record.Id
					if houseId and owner then
						local openHouseData = self:GetOpenHouse(houseId, owner)
						if openHouseData then
							record.HouseCategory = openHouseData.C
						end
					end
				end
			end
...
function EHH:ShowProgressIndicatorDialog(caption)
	local ui = self:GetDialog("ProgressIndicator")
	if not ui then
		ui = self:CreateDialog("ProgressIndicator")

		local prefix = "EHHProgressIndicatorDialog"
		...
		local function tip(control, msg)
			self:SetTooltip(msg, control)
			--self:SetInfoTooltip(control, msg)
		end
		...
		win:SetHandler("OnMoveStop", function() self:SaveDialogSettings(prefix, win) end)

		c = CreateLabel(nil, win, "Essential Housing", CreateAnchor(BOTTOMLEFT, win, TOPLEFT, 0, 0), CreateAnchor(BOTTOMRIGHT, win, TOPRIGHT, 0, 0), nil, nil, TEXT_ALIGN_CENTER)
		ui.Heading = c
		...
		ui.CancelButton = CreateButton(prefix .. "CancelButton", win, "Cancel", CreateAnchor(TOP, ui.Body, BOTTOM, 0, 8), 80, 24, function(...) return self:CancelProcess(...) end)
	...
	local separateBar = self:GetSetting("ShowSeparateProgressBar")
	ui.Window:SetHidden(true == EHH.IsUIHidden or false == separateBar)

	return ui
end

function EHH:UpdateProgressIndicator(caption, progress)
	progress = zo_clamp(tonumber(progress) or 0, 0, 1)

	local ui = self:ShowProgressIndicatorDialog()
...
-- /script HousingHubStreamMessage:Play( "|cffff44So and So|r\nis live on Twitch", "")
