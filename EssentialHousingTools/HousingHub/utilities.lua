...
function EHH:GetInGameTime()
	if self.InGameTimeOverride then
		return self.InGameTimeOverride.Hours, self.InGameTimeOverride.Minutes, 0
	end

	local timeDefs = self.Defs.InGameTime
	local offsetTime = GetTimeStamp() - timeDefs.GameTimeOffset
	local gameTime = offsetTime % timeDefs.RealSecondsPerGameDay
	local hours = math.floor((gameTime / timeDefs.RealSecondsPerGameHour) % 24)
	local minutes = math.floor((gameTime / timeDefs.RealSecondsPerGameMinute) % 60)
	local seconds = math.floor((gameTime / timeDefs.RealSecondsPerGameSecond) % 60)
...
function EHH:GetInGameTimeString()
	local h, m, s = self:GetInGameTime()
	local half = "AM"
	...
	return string.format("Tamriel time is %.2d:%.2d:%.2d %s.", h, m, s, half)
end
