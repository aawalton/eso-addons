...
do
	...
	function EHH:QueueQueryStatefulFurniture()
		if not isQueued then
			isQueued = true
			HUB_EVENT_MANAGER:RegisterForUpdate(self.Name .. "FurnitureStateQuery", self.Defs.Limits.QueryFurnitureStateIntervalMS, function() self:QueryStatefulFurniture() end)
		end
	end

	function EHH:QueryStatefulFurniture()
		if not self:IsHouseZone() then
			isQueued = false
			HUB_EVENT_MANAGER:UnregisterForUpdate(self.Name .. "FurnitureStateQuery")
			return
		end

		for numProcessed = 1, 5 do
			currentQueueKey = next(self.FurnitureStates, currentQueueKey)
			if not currentQueueKey then
				return
			end

			local changed, currentState, previousState = self:HasFurnitureStateChanged(currentQueueKey)
			if changed then
				self:OnFurnitureStateChanged(currentQueueKey, currentState, previousState)
			end
		end
	end
end

function EHH:ClearCachedFurnitureState(furnitureId)
	local key = self:FromId64(furnitureId)
	if not key then
		return false
	end
	
	self.FurnitureStates[key] = nil
	return true
end

function EHH:CacheInitialFurnitureState(furnitureId)
	local key = self:FromId64(furnitureId)
	if not key then
		return false
	end

	local currentState = self:GetFurnitureState(furnitureId)
	self.FurnitureStates[key] = currentState
	return true
end

function EHH:CacheAllInitialFurnitureStates()
	self.FurnitureStates = {}
	self.FurnitureStateTimestamps = {}

	local furnitureId = GetNextPlacedHousingFurnitureId()
	while furnitureId do
		self:CacheInitialFurnitureState(furnitureId)
		furnitureId = GetNextPlacedHousingFurnitureId(furnitureId)
	end
...
function EHH:HasFurnitureStateChanged(furnitureId)
	local key = self:FromId64(furnitureId)
	if not key then
		return false
	end

	local previousState = self.FurnitureStates[key]
	local currentState = self:GetFurnitureState(furnitureId)
	self.FurnitureStates[key] = currentState
...
function EHH:GetFurnitureState(furnitureId)
	...
	if not state then
		if self:IsEffectId(furnitureId) then return nil end
		...
		if self:DoesFurnitureHaveIndexableStateNames(furnitureId) then
			...
			state = self:GetFurnitureStateByStateName(stateName, furnitureId)
		end

		if not state then
			state = self.Defs.DefaultFurnitureStates[stateIndex + 1]
		end
	end
...
-- TODO
--[[
	local record = self:GetEffectRecordById(id)
	if not record then return nil end

	local effectType = EHH.EffectType:GetByIndex(record.EffectType)
	return effectType
end
]]
...
--[[
	...
	local id = tonumber(itemId) - self.Defs.BaseEffectItemTypeId
	return EHH.EffectType:GetByIndex(id)
end
]]
