...
function EHH:OnPlayerActivated(_, initialActivation)
	...
	if didZoneChange then
		if self.Defs.EnableEffects then
			...
			self:CacheAllInitialFurnitureStates()
			self:QueueQueryStatefulFurniture()
		end
...
function EHH:OnCurrentHouseChanged(isInHouse)
	...
	if isInHouse then
		...
		local housePopulation = self:GetHousePopulation()
		if housePopulation > 1 then
			self:OnHouseGuestsPresent(housePopulation)
		end
	end
...
function EHH:OnFurnitureStateChanged(id, newState, oldState)
	if self.Defs.EnableEffects then
		EHH.EffectFurnitureStateManager:OnFurnitureStateChanged(id, newState, oldState)
	end
end

function EHH:OnChatMessage(_, channel, from, text, isCustServ, displayName)
	if self.IsEHT then
		HUB_EVENT_MANAGER:UnregisterForEvent(self.Name, EVENT_CHAT_MESSAGE_CHANNEL)
	elseif text then
		self:ParseHouseLinks(text)
	end
end
...
function EHH:OnLinkClicked(rawLinkString, mouseButton, linkText, linkStyle, linkType, owner, houseId)
	if self.IsEHT then
		HUB_EVENT_MANAGER:UnregisterCallback(LINK_HANDLER, LINK_HANDLER.LINK_MOUSE_UP_EVENT, self.OnLinkClickedCallback)
	elseif linkType then
		linkType = string.lower(tostring(linkType))

		if "house" == linkType then
			self:OnHouseLinkClicked(houseId, owner)
			return true
		end
	end
end
...
function EHH:OnFurnitureRemoved(_, furnitureId, collectibleId)
	if furnitureId and 0 ~= furnitureId then
		self:ClearCachedFurnitureState(furnitureId)
		if self.Defs.EnableEffects then
...
function EHH:RegisterEventHandlers()
	if not self.RegisteredEventHandlers then
		...
		HUB_EVENT_MANAGER:RegisterForEvent(self.Name, EVENT_CHAT_MESSAGE_CHANNEL, function(...) return self:OnChatMessage(...) end)
		...
		HUB_EVENT_MANAGER:RegisterCallback(LINK_HANDLER, LINK_HANDLER.LINK_MOUSE_UP_EVENT, self.OnLinkClickedCallback)
