...
local MSG_SELECTION_EMPTY = "|cffffff" ..
	...
	"While in Housing Editor mode, selected items are identified by a rotating \"check\" icon."
...
do
	...
	local function ControlFactory( pool )
		local control = WINDOW_MANAGER:CreateControl( nil, pool.parent, pool.controlType )
...
function EHT.UI.SetupUIExtensions()
	...
	do
		...
		if target then
			c = WINDOW_MANAGER:CreateControl( prefix .. "FurniturePlacementInstructions", target, CT_LABEL )
	...
	do
		...
		if target then
			c = WINDOW_MANAGER:CreateControl( prefix .. "FurnitureRetrievalInstructions", target, CT_LABEL )
	...
	do
		local target = ZO_HousingFurniturePlacementPanel_KeyboardTopLevel
		if target then
			local c

			do
				local filterLabelControl = WINDOW_MANAGER:CreateControl( nil, target, CT_LABEL )
				target.EssentialPlacementFiltersLabel = filterLabelControl
				filterLabelControl:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thin" )
				filterLabelControl:SetColor( 1, 1, 1, 1 )
				filterLabelControl:SetAnchor( TOPLEFT, target, TOPLEFT, 300, 5 )
				filterLabelControl:SetHorizontalAlignment( TEXT_ALIGN_LEFT )
			end

			local function CreateHighlightControl( controlName )
				local c = WINDOW_MANAGER:CreateControl( controlName, target, CT_TEXTURE )
				c:SetHidden( true )
				c:SetTexture( EHT.Textures.CIRCLE_SOFT_2 )
				c:SetDimensions( 60, 60 )
				c:SetColor( 0.45, 0.2, 0.9, 0.4 )
				c:SetMouseEnabled( false )
				c:SetHandler( "OnEffectivelyHidden", function( control )
					EVENT_MANAGER:UnregisterForUpdate( control:GetName() .. "Flash" )
				end )
				c:SetHandler( "OnEffectivelyShown", function( control )
					EVENT_MANAGER:RegisterForUpdate( control:GetName() .. "Flash", 1, function()
						local interval = ZO_EaseInCubic( math.sin( ( GetFrameTimeMilliseconds() % 2000 ) / 2000 * math.pi ) )
						control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_RGB, 1 + 3 * interval )
					end )
				end )
				return c
			end
			...
			local highlightedControls = { }
			local highlightControls = {
				["bag"] = CreateHighlightControl( "EssentialPlacementFilterBagHighlight" ),
				["bound"] = CreateHighlightControl( "EssentialPlacementFilterBoundHighlight" ),
			}

			target.filterMessages = filterMessages
			target.filterControls = filterControls
			target.highlightedControls = highlightedControls
			target.highlightControls = highlightControls

			local function AddFilterControl( filterType, control )
				control:SetDimensions( 32, 32 )
				control:SetColor( 1, 1, 1, 1 )
				control:SetDesaturation( 1 )
				control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_ALPHA_AS_RGB, 0.85 )
				control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_RGB, 0.5 )
				control:SetMouseEnabled( true )
				table.insert( filterControls[filterType], control )
			end

			local function UpdateEssentialFiltersLabel()
				local bagFilterMessage = filterMessages["bag"]
				local boundFilterMessage = filterMessages["bound"]
				local prefixMessage = bagFilterMessage and "" or " all"

				boundFilterMessage = boundFilterMessage and " " .. boundFilterMessage or ""
				bagFilterMessage = bagFilterMessage and " " .. bagFilterMessage or ""
				target.EssentialPlacementFiltersLabel:SetText( string.format( "Essential Filter: |c33ffffShow collectibles and%s%s%s items|r", prefixMessage, boundFilterMessage, bagFilterMessage ) )
			end

			local function HighlightFilter( filterType, filterControl, filterMessage )
				local filterControlList = filterControls[filterType]
				local filterHighlightControl = highlightControls[filterType]

				if filterHighlightControl then
					filterHighlightControl:ClearAnchors()
					filterHighlightControl:SetHidden( true )
				end

				if filterControlList then
					for _, control in ipairs( filterControlList ) do
						if filterControl == control then
							if filterHighlightControl then
								filterHighlightControl:SetAnchor( CENTER, control, CENTER )
								filterHighlightControl:SetHidden( false )
							end
							control:SetColor( 1, 1, 1, 1 )
							control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_ALPHA_AS_RGB, 0.5 )
							control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_RGB, 0.85 )
							tip( control, "|cffff88Clear this filter", TOPLEFT, nil, 10, BOTTOM )
						else
							control:SetColor( 0.5, 0.5, 0.5, 1 )
							control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_ALPHA_AS_RGB, 0.85 )
							control:SetTextureSampleProcessingWeight( TEX_SAMPLE_PROCESSING_RGB, 0.5 )
							tip( control, control.normalTooltip, TOPLEFT, nil, 10, BOTTOM )
						end
					end
				end

				filterMessages[filterType] = highlightedControls[filterType] and filterMessage
				UpdateEssentialFiltersLabel()
			end

			local function ToggleFilter( filterType, control, filterMessage, filterFunction, clearFunction )
				local currentControl = highlightedControls[filterType]
				if currentControl == control then
					highlightedControls[filterType] = nil
					HighlightFilter( filterType )
					if clearFunction then clearFunction() end
				else
					highlightedControls[filterType] = control
					HighlightFilter( filterType, control, filterMessage )
					if filterFunction then filterFunction() end
				end
			end
			...
			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( TOPLEFT, target.EssentialPlacementFiltersLabel, BOTTOMLEFT, 0, 10 )
			c:SetTexture( "esoui/art/tutorial/gamepad/gp_icon_locked32.dds" )
			c.normalTooltip = string.format( "List only Bound %s items", EHT.ICON_CROWN )
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bound", control, "bound", EHT.UI.FilterPlaceableFurnitureByBound, EHT.UI.ClearBoundFilterPlaceableFurniture )
			end )
			AddFilterControl( "bound", c )
			prev = c

			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( LEFT, prev, RIGHT, 15, 0 )
			c:SetTexture( "esoui/art/tutorial/gamepad/gp_icon_unlocked32.dds" )
			c.normalTooltip = string.format( "List only Tradeable (Unbound) %s items", EHT.ICON_TRADEABLE )
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bound", control, "unbound", EHT.UI.FilterPlaceableFurnitureByUnbound, EHT.UI.ClearBoundFilterPlaceableFurniture )
			end )
			AddFilterControl( "bound", c )
			prev = c
			...
			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( LEFT, prev, RIGHT, 15, 0 )
			c:SetTexture( EHT.Textures.GRADIENT_CENTER )
			c:SetDimensions( 5, 42 )
			...
			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( LEFT, prev, RIGHT, 15, 0 )
			c:SetTexture( "esoui/art/tooltips/icon_bag.dds" )
			c.normalTooltip = "List only items in your inventory"
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bag", control, "inventory", EHT.UI.FilterPlaceableFurnitureByBackpack, EHT.UI.ClearPlaceableFurnitureFilter )
			end )
			AddFilterControl( "bag", c )
			c:SetDimensions( 30, 30 )
			prev = c

			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( LEFT, prev, RIGHT, 15, 0 )
			c:SetTexture( "esoui/art/icons/servicemappins/servicepin_bank.dds" )
			c.normalTooltip = "List only items in your bank"
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bag", control, "bank", EHT.UI.FilterPlaceableFurnitureByBank, EHT.UI.ClearPlaceableFurnitureFilter )
			end )
			AddFilterControl( "bag", c )
			prev = c
			...
			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( LEFT, prev, RIGHT, 15, 0 )
			c:SetTexture( "esoui/art/icons/store_upgrade_bank.dds" )
			c.normalTooltip = "List only items in your placed storage chests & coffers"
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bag", control, "storage chest/coffer", EHT.UI.FilterPlaceableFurnitureByHouseBanks, EHT.UI.ClearPlaceableFurnitureFilter )
			end )
			AddFilterControl( "bag", c )
	...
	do
		local target = ZO_HousingFurnitureRetrievalPanel_KeyboardTopLevel
		if target then
			local c

			do
				local filterLabelControl = WINDOW_MANAGER:CreateControl( nil, target, CT_LABEL )
				target.EssentialRetrievableFiltersLabel = filterLabelControl
				filterLabelControl:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thin" )
				filterLabelControl:SetColor( 1, 1, 1, 1 )
				filterLabelControl:SetAnchor( TOPLEFT, target, TOPLEFT, 300, 5 )
				filterLabelControl:SetHorizontalAlignment( TEXT_ALIGN_LEFT )
			...
			local highlightedControls = { }
			local highlightControls = {
				["bound"] = CreateHighlightControl( "EssentialRetrievableFilterBoundHighlight" ),
			}
			...
			local function UpdateEssentialFiltersLabel()
				local boundFilterMessage = filterMessages["bound"]
				boundFilterMessage = boundFilterMessage or "all"
				target.EssentialRetrievableFiltersLabel:SetText( string.format( "Essential Filter: |c33ffffShow collectibles and %s items|r", boundFilterMessage ) )
			end
			...
			c = WINDOW_MANAGER:CreateControl( nil, target, CT_TEXTURE )
			c:SetAnchor( TOPLEFT, target.EssentialRetrievableFiltersLabel, BOTTOMLEFT, 0, 10 )
			c:SetTexture( "esoui/art/tutorial/gamepad/gp_icon_locked32.dds" )
			c.normalTooltip = string.format( "List only Bound %s items", EHT.ICON_CROWN )
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bound", control, "bound", EHT.UI.FilterRetrievableFurnitureByBound, EHT.UI.ClearBoundFilterRetrievableFurniture )
			end )
			AddFilterControl( "bound", c )
			prev = c
			...
			c.normalTooltip = string.format( "List only Tradeable (Unbound) %s items", EHT.ICON_TRADEABLE )
			c:SetHandler( "OnMouseDown", function( control )
				ToggleFilter( "bound", control, "unbound", EHT.UI.FilterRetrievableFurnitureByUnbound, EHT.UI.ClearBoundFilterRetrievableFurniture )
			end )
			AddFilterControl( "bound", c )
			prev = c
...
function EHT.UI.TabTargetFurniture()

	return EHT.Biz.TabTargetFurniture()
...
function EHT.UI.OnHouseGuestsPresent( population )
	local numGuests = population - 1
	local plural = 1 == numGuests and "" or "s"
	local verb = 1 == numGuests and "is" or "are"
	local houseName = EHT.Housing.GetHouseName()
	local houseOwner = EHT.Housing.GetHouseOwner()

	if EHT.Housing.IsOwner() then
		EHT.UI.ShowPopulationNotification( string.format( "%d guest%s %s here in your %s.", numGuests, plural, verb, houseName ) )
	else
		EHT.UI.ShowPopulationNotification( string.format( "%d other guest%s %s here in %s's %s.", numGuests, plural, verb, houseOwner, houseName ) )
	end
end
...
function EHT.UI.OnHousePopulationChanged( population )
	...
	local maxPopulation = GetCurrentHousePopulationCap() or 0

	if population > previousPopulation then
		if isOwner then
			EHT.UI.ShowPopulationNotification( string.format( "A guest has arrived to your %s. (%d of %d max)", houseName, population, maxPopulation ) )
		else
			EHT.UI.ShowPopulationNotification( string.format( "A guest has arrived to %s's %s. (%d of %d max)", houseOwner, houseName, population, maxPopulation ) )
		end
	else
		if isOwner then
			EHT.UI.ShowPopulationNotification( string.format( "A guest has departed from your %s. (%d of %d max)", houseName, population, maxPopulation ) )
		else
			EHT.UI.ShowPopulationNotification( string.format( "A guest has departed from %s's %s. (%d of %d max)", houseOwner, houseName, population, maxPopulation ) )
		end
	end
	...
	if EHT.ChooseItemCallback and mode == HOUSING_EDITOR_MODE_SELECTION then
		...
		if not EHT.SavedVars.HideKeybinds then
			if not EHT.ChooseItemCallback and mode == HOUSING_EDITOR_MODE_SELECTION then
				EHT.UI.AddKeybindButton( EHT.KEYBIND_BUTTONS.TAB_TARGET )
...
function EHT.UI.ParseHouseLinks( text )
	local tagStartIndex, tagEndIndex = string.find( text, "%[Visit " )
	if tagEndIndex then
		local ownerStartIndex = tagEndIndex + 1
		local ownerEndIndex = string.find( text, "%,", tagEndIndex )
		if ownerEndIndex then
			local ownerName = string.sub( text, ownerStartIndex, ownerEndIndex - 1 )
			local houseNameStartIndex = ownerEndIndex + 1
			local houseNameEndIndex = string.find( text, "%]", houseNameStartIndex )
			if houseNameEndIndex then
				local houseName = EHT.Util.Trim( string.sub( text, houseNameStartIndex, houseNameEndIndex - 1 ) )
				local houseData = EHT.Housing.GetHouseByName( houseName )
				local houseId
				if houseData then
				...
				-- Display the interactive link to jump to this house.
				d( EHT.UI.CreateHouseLink( ownerName, houseId ) )
			end
		end
	end
end
...
function EHT.UI.CreateHouseLink( ownerName, houseId )
	...
	if "@" == string.sub( ownerName, 1, 1 ) then
		ownerName = string.sub( ownerName, 2 )
	end
	if "" == ownerName then
		return ""
	end
	...
	if not houseName or "" == houseName then
		houseName = "Primary Home"
	end

	return string.format( "|H0:house:%s:%d|h[Click here to visit %s's %s]|h", ownerName, houseId, DecorateDisplayName( ownerName ), houseName )
end

function EHT.UI.CreateHouseLinkSymbol( ownerName, houseId )
	houseId = tonumber( houseId ) or 0
	...
	return string.format( "[Visit %s, %s]", DecorateDisplayName( ownerName ), houseName )
	-- return string.format( "|H0:house:%s:%d|h[Visit %s's %s]|h", ownerName, houseId, DecorateDisplayName( ownerName ), houseName )
	-- return ZO_LinkHandler_CreateLink( "Visit Home", nil, "house", ownerName, houseId )
	-- ZO_LinkHandler_CreateLink( string.format( "Visit %s's %s", ownerName, houseName ), nil, "house", ownerName, houseId )
end

function EHT.UI.ShareHouseLink( ownerName, houseId )
	if CHAT_SYSTEM then
		local link = EHT.UI.CreateHouseLinkSymbol( ownerName, houseId )
...
function EHT.UI.OnHouseLinkClicked( ownerName, houseId )
	ownerName = tostring( ownerName ) or ""
	if "" == ownerName then
		return
	end
	ownerName = string.format( "@%s", ownerName )
	...
	if 0 == houseId then
		...
		houseName = GetCollectibleName( GetCollectibleIdForHouse( houseId ) )
	end
	if not houseName or "" == houseName then
		houseName = "primary home"
	end

	local prep = "the " == string.sub( string.lower( houseName ), 1, 4 ) and "" or "the "
	local message = string.format( "Visit %s|c88ffff%s|r of |c88ffff%s|r?", prep, houseName, ownerName )
	local function jumpFunction()
		EHT.Housing.JumpToHouse( houseId, ownerName, false )
	end

	EHT.UI.ShowConfirmationDialog( "", message, jumpFunction )
end
...
function EHT.UI.SetupToolDialogBuildTab( ui, parent, prefix, settings )
	...
	ui.BuildHeaderGroup = WINDOW_MANAGER:CreateControl( prefix .. "BuildHeaderGroup", parent, CT_CONTROL )
	...
	ui.BuildFunctionGroup = WINDOW_MANAGER:CreateControl( prefix .. "BuildFunctionGroup", parent, CT_CONTROL )
    ...
    local cbg = WINDOW_MANAGER:CreateControl( nil, parent, CT_BACKDROP )
	...
	ui.BuildParamsContainer = WINDOW_MANAGER:CreateControl( prefix .. "BuildParamsSection", cbg, CT_SCROLL )
	...
	ui.BuildParamsSection = WINDOW_MANAGER:CreateControl( prefix .. "BuildParamsContainer", ui.BuildParamsContainer, CT_CONTROL )
	...
	ui.BuildParamsScrollSlider = WINDOW_MANAGER:CreateControl( prefix .. "BuildParamsScrollSlider", parent, CT_SLIDER )
    ...
    local bg = WINDOW_MANAGER:CreateControl( nil, ui.BuildParamsScrollSlider, CT_BACKDROP )
	...
	local msg = WINDOW_MANAGER:CreateControl( prefix .. "BuildMessageParam", bps, CT_CONTROL )
	bp.Message = msg
	msg:SetDimensions( width, 50 )
	msg.IsParam = true

	msg.Label = WINDOW_MANAGER:CreateControl( nil, msg, CT_LABEL ) 
	...
	bp.EllipseParam = WINDOW_MANAGER:CreateControl( prefix .. "BuildEllipseParam", bps, CT_CONTROL )
	...
	bp.ItemSpacingAuto = WINDOW_MANAGER:CreateControl( prefix .. "BuildItemSpacingAuto", bps, CT_CONTROL )
	...
	bp.CheckerPattern = WINDOW_MANAGER:CreateControl( prefix .. "BuildCheckerPattern", bps, CT_CONTROL )
	...
	bp.ReverseSort = WINDOW_MANAGER:CreateControl( prefix .. "BuildReverseSort", bps, CT_CONTROL )
...
local DIRECTIONAL_PAD_OFFSET = 35
...
function EHT.UI.UpdateDirectionalPadHeading()
	...
	if EHT.Biz.IsCardinalPositionMode() then
		...
		local x, y
		x, y = math.sin( heading ), math.cos( heading )
		ui.DirectionalModeSouth:SetAnchor( CENTER, button, CENTER, x * DIRECTIONAL_PAD_OFFSET, y * DIRECTIONAL_PAD_OFFSET )
		ui.DirectionalModeSouth:SetScale( 1.5 - 0.325 * ( 1 + y ) )

		x, y = math.sin( heading + RAD90 ), math.cos( heading + RAD90 )
		ui.DirectionalModeEast:SetAnchor( CENTER, button, CENTER, x * DIRECTIONAL_PAD_OFFSET, y * DIRECTIONAL_PAD_OFFSET )
		ui.DirectionalModeEast:SetScale( 1.5 - 0.325 * ( 1 + y ) )

		x, y = math.sin( heading + RAD180 ), math.cos( heading + RAD180 )
		ui.DirectionalModeNorth:SetAnchor( CENTER, button, CENTER, x * DIRECTIONAL_PAD_OFFSET, y * DIRECTIONAL_PAD_OFFSET )
		ui.DirectionalModeNorth:SetScale( 1.5 - 0.325 * ( 1 + y ) )

		x, y = math.sin( heading - RAD90 ), math.cos( heading - RAD90 )
		ui.DirectionalModeWest:SetAnchor( CENTER, button, CENTER, x * DIRECTIONAL_PAD_OFFSET, y * DIRECTIONAL_PAD_OFFSET )
		ui.DirectionalModeWest:SetScale( 1.5 - 0.325 * ( 1 + y ) )
	else
...
function EHT.UI.SetupToolDialogEditTab( ui, parent, prefix, settings, source )
	...
	local function addButton(label, parent, directionOrientation, onClick)
		local b = WINDOW_MANAGER:CreateControl(nil, parent, CT_TEXTURE)
	...
	ui.DirectionalControls = WINDOW_MANAGER:CreateControl( prefix .. "DirectionalControls", parent, CT_CONTROL )
	...
	ui.RelationalGroup = WINDOW_MANAGER:CreateControl( prefix .. "RelationalGroup", ui.DirectionalControls, CT_CONTROL )
	...
	if not sourceEditFurniture then 
		ui.CardinalGroup = WINDOW_MANAGER:CreateControl( prefix .. "CardinalGroup", ui.DirectionalControls, CT_CONTROL )
	...
	ui.PitchRollGroup = WINDOW_MANAGER:CreateControl( prefix .. "PitchRollGroup", ui.DirectionalControls, CT_CONTROL )
	...
	ui.RotationalGroup = WINDOW_MANAGER:CreateControl( prefix .. "RotationalGroup", ui.DirectionalControls, CT_CONTROL )
	...
	ui.EditSettingsGroup = WINDOW_MANAGER:CreateControl( prefix .. "EditSettingsGroup", parent, CT_CONTROL )
	...
	ui.PrecisionBackdrop = WINDOW_MANAGER:CreateControl( nil, ui.Precision, CT_BACKDROP )
	...
	ui.PrecisionLabel = WINDOW_MANAGER:CreateControl( nil, ui.Precision, CT_LABEL )
	...
	ui.PrecisionMinLabel = WINDOW_MANAGER:CreateControl( nil, ui.Precision, CT_LABEL )
	...
	ui.PrecisionMaxLabel = WINDOW_MANAGER:CreateControl( nil, ui.Precision, CT_LABEL )
	...
	do
		...
		b:SetHandler( "OnEffectivelyShown", function() EVENT_MANAGER:RegisterForUpdate( "EHT.UI.UpdateDirectionalPadHeading", 20, EHT.UI.UpdateDirectionalPadHeading ) end )
...
function EHT.UI.SetupToolDialogSelectionTab( ui, parent, prefix, settings )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "LoadSelectionContainer", parent, CT_CONTROL )
	ui.LoadSelectionContainer = grp
	grp:SetAnchor( TOPLEFT, parent, TOPLEFT, 0, 0 )
	grp:SetAnchor( BOTTOMRIGHT, parent, TOPRIGHT, 0, 51 )

	c = WINDOW_MANAGER:CreateControl( prefix .. "LoadSelectionLabel", grp, CT_LABEL )
...
--[[
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "SelectionHeaderGroup", parent, CT_CONTROL )
	...
	do
		...
		c = WINDOW_MANAGER:CreateControl( nil, g, CT_LABEL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "SelectionActionsGroupContainer", parent, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "SelectionStatsContainer", parent, CT_CONTROL )
	...
	do
		...
		c = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
...
--[[
	ui.Buffer = WINDOW_MANAGER:CreateControl( prefix .. "Buffer", parent, CT_TEXTBUFFER )
	...
	ui.Slider = WINDOW_MANAGER:CreateControl( prefix .. "Slider", parent, CT_SLIDER )
...
function EHT.UI.SetupToolDialogClipboardTab( ui, parent, prefix, settings )
	...
	ui.ClipboardActionsGroupContainer = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardActionsGroupContainer", parent, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardCopyActionsGroup", ui.ClipboardActionsGroupContainer, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardPasteActionsGroup", ui.ClipboardActionsGroupContainer, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardOtherActionsGroup", ui.ClipboardActionsGroupContainer, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardContainer", parent, CT_CONTROL )
	...
	ctl = WINDOW_MANAGER:CreateControl( nil, grp, CT_BACKDROP )
	...
	ctl = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardBuffer", grp, CT_TEXTBUFFER )
	...
	ui.ClipboardSlider = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardSlider", grp, CT_SLIDER )
...
function EHT.UI.SetupToolDialogAnimateTab( ui, parent, prefix, settings )
	...
	ui.EditSceneGroup = WINDOW_MANAGER:CreateControl( prefix .. "EditSceneGroup", parent, CT_CONTROL )
	...
	do
		...
		ui.ManageScenesGroup = WINDOW_MANAGER:CreateControl( prefix .. "ManageScenesGroup", ui.EditSceneGroup, CT_CONTROL )
		...
		ui.CurrentScene = WINDOW_MANAGER:CreateControl( prefix .. "CurrentScene", ui.ManageScenesGroup, CT_LABEL )
		...
		local divider = WINDOW_MANAGER:CreateControl( nil, ui.EditSceneGroup, CT_TEXTURE )
		divider:SetHeight( 5 )
		...
		ui.FrameGroup = WINDOW_MANAGER:CreateControl( prefix .. "FrameGroup", ui.EditSceneGroup, CT_CONTROL )
		...
		ui.FrameIndex = WINDOW_MANAGER:CreateControl( prefix .. "FrameIndex", ui.FrameGroup, CT_SLIDER )
		...
		ui.FrameIndexBackdrop = WINDOW_MANAGER:CreateControl( nil, ui.FrameGroup, CT_BACKDROP )
		...
		ui.FrameIndexLabel = WINDOW_MANAGER:CreateControl( nil, ui.FrameGroup, CT_LABEL )
		...
		ui.FrameIndexMinLabel = WINDOW_MANAGER:CreateControl( nil, ui.FrameGroup, CT_LABEL )
		...
		ui.FrameIndexMaxLabel = WINDOW_MANAGER:CreateControl( nil, ui.FrameGroup, CT_LABEL )
		...
		ui.ManageFramesGroup = WINDOW_MANAGER:CreateControl( prefix .. "ManageFramesGroup", ui.EditSceneGroup, CT_CONTROL )
		...
		ui.FrameDetailsGroup = WINDOW_MANAGER:CreateControl( prefix .. "FrameDetailsGroup", ui.EditSceneGroup, CT_CONTROL )
		...
		ui.FrameDurationGroup = WINDOW_MANAGER:CreateControl( prefix .. "FrameDurationGroup", ui.FrameDetailsGroup, CT_CONTROL )
		ui.FrameDurationGroup:SetAnchor( TOP, ui.FrameDetailsGroup, TOP, 0, 0 )
		ui.FrameDurationGroup:SetResizeToFitDescendents( true )

		ui.FrameDurationLabel = WINDOW_MANAGER:CreateControl( nil, ui.FrameDurationGroup, CT_LABEL )
		...
		ui.FrameDurationLabel2 = WINDOW_MANAGER:CreateControl( nil, ui.FrameDurationGroup, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TestFrameSound", ui.FrameDetailsGroup, CT_TEXTURE )
		ui.TestFrameSound = c
		...
		div = WINDOW_MANAGER:CreateControl( nil, parent, CT_TEXTURE )
		div:SetHeight( 5 )
		div:SetAlpha( 0.5 )
	...
	ui.LoadSceneGroup = WINDOW_MANAGER:CreateControl( prefix .. "LoadSceneGroup", parent, CT_CONTROL )
	...
	do
		...
		ui.LoadSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.LoadSceneGroup, CT_LABEL )
	...
	ui.SaveSceneGroup = WINDOW_MANAGER:CreateControl( prefix .. "SaveSceneGroup", parent, CT_CONTROL )
	...
	do
		...
		ui.SaveSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.SaveSceneGroup, CT_LABEL )
		...
		ui.SaveAsSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.SaveSceneGroup, CT_LABEL )
	...
	ui.MergeSceneGroup = WINDOW_MANAGER:CreateControl( prefix .. "MergeSceneGroup", parent, CT_CONTROL )
	...
	do
		...
		ui.MergeSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.MergeSceneGroup, CT_LABEL )
		...
		ui.MergeAsSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.MergeSceneGroup, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( nil, ui.MergeSceneGroup, CT_CONTROL )
		ui.MergeSceneLoopedContainer = c
		c:SetAnchor( TOP, ui.MergeSceneNameBackdrop, BOTTOM, 0, 15 )
		c:SetResizeToFitDescendents( true )

		c = WINDOW_MANAGER:CreateControl( nil, ui.MergeSceneLoopedContainer, CT_LABEL )
	...
	ui.AppendSceneGroup = WINDOW_MANAGER:CreateControl( prefix .. "AppendSceneGroup", parent, CT_CONTROL )
	...
	do
		...
		ui.AppendSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.AppendSceneGroup, CT_LABEL )
		...
		ui.AppendAsSceneLabel = WINDOW_MANAGER:CreateControl( nil, ui.AppendSceneGroup, CT_LABEL )
...
function EHT.UI.SetupToolDialogTriggerTab( ui, parent, prefix, settings )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerContainer", parent, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerActionsContainer", grp, CT_CONTROL )
	...
	div = WINDOW_MANAGER:CreateControl( nil, grp, CT_TEXTURE )
	div:SetHeight( 5 )
	div:SetAlpha( 0.5 )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerScroll", ui.TriggerContainer, CT_SCROLL )
	...
	c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerScrollBackground1", ui.TriggerContainer, CT_TEXTURE )
	...
	c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerScrollBackground2", ui.TriggerContainer, CT_TEXTURE )
	...
	c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerScrollBackground3", ui.TriggerContainer, CT_TEXTURE )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerScrollContainer", ui.TriggerScroll, CT_CONTROL )
	...
	c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerSlider", ui.TriggerContainer, CT_SLIDER )
	ui.TriggerSlider = c
	...
	local function addCondition( index, prefix, ui, parent, caption )
		...
		c = WINDOW_MANAGER:CreateControl( nil, parent, CT_CONTROL )
		ui.TriggerConditionTypeLabel = c
		c:SetAnchor( TOPLEFT, parent, TOPLEFT, 0, 0 )
		c:SetAnchor( TOPRIGHT, parent, TOPRIGHT, 0, 0 )
		c:SetResizeToFitDescendents( true )

		c = WINDOW_MANAGER:CreateControl( nil, ui.TriggerConditionTypeLabel, CT_LABEL )
		c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
		SetColor(c, Colors.White)
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionContainer", parent, CT_CONTROL )
		...
		sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionItemContainer", ui.TriggerConditionContainer, CT_CONTROL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionItemIcon", sgrp, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionItem", sgrp, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( nil, sgrp, CT_LABEL )
		...
		sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionPhraseContainer", ui.TriggerConditionContainer, CT_CONTROL )
		...
		sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionPositionContainer", ui.TriggerConditionContainer, CT_CONTROL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionPosition", sgrp, CT_LABEL )
...
--[[
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionPositionValid", sgrp, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionPositionInvalid", sgrp, CT_LABEL )
		...
		sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionEmoteContainer", ui.TriggerConditionContainer, CT_CONTROL )
		...
		sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionQuickslotContainer", ui.TriggerConditionContainer, CT_CONTROL )
		ui.TriggerConditionQuickslotContainer = sgrp
		sgrp:SetAnchor( TOPLEFT, ui.TriggerConditionContainer, TOPLEFT, 20, 0 )
		sgrp:SetAnchor( TOPRIGHT, ui.TriggerConditionContainer, TOPRIGHT, 0, 0 )
		sgrp:SetResizeToFitDescendents( true )

		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionQuickslotLabel", sgrp, CT_LABEL )
		ui.TriggerConditionQuickslotLabel = c
		c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
		c:SetAnchor( TOPLEFT, sgrp, TOPLEFT, 0, 0 )
		c:SetText( "Item:" )

		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionQuickslotLink", sgrp, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionQuickslotInstructions", sgrp, CT_LABEL )
		...
		sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionInteractContainer", ui.TriggerConditionContainer, CT_CONTROL )
		ui.TriggerConditionInteractContainer = sgrp
		sgrp:SetAnchor( TOPLEFT, ui.TriggerConditionContainer, TOPLEFT, 20, 0 )
		sgrp:SetAnchor( TOPRIGHT, ui.TriggerConditionContainer, TOPRIGHT, 0, 0 )
		sgrp:SetResizeToFitDescendents( true )

		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionInteractLabel", sgrp, CT_LABEL )
		ui.TriggerConditionInteractLabel = c
		c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
		c:SetAnchor( TOPLEFT, sgrp, TOPLEFT, 0, 0 )
		c:SetText( "Interaction Target:" )

		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionInteractTarget", sgrp, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "TriggerConditionInteractInstructions", sgrp, CT_LABEL )
	...
	grp = WINDOW_MANAGER:CreateControl( nil, ui.TriggerScrollContainer, CT_CONTROL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerActionContainer", ui.TriggerScrollContainer, CT_CONTROL )
	...
	sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerActionGroupContainer", ui.TriggerActionContainer, CT_CONTROL )
	...
	sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerActionSceneContainer", ui.TriggerActionContainer, CT_CONTROL )
	...
	sgrp = WINDOW_MANAGER:CreateControl( prefix .. "TriggerActionTriggerContainer", ui.TriggerActionContainer, CT_CONTROL )
...
function EHT.UI.SetupToolDialogToolTab( ui, parent, prefix, settings )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "HouseInfoSection", parent, CT_CONTROL )
	ui.HouseInfoSection = grp
	grp:SetAnchor( TOPLEFT, parent, TOPLEFT, 0, 5 )
	grp:SetResizeToFitDescendents( true )

	c = WINDOW_MANAGER:CreateControl( prefix .. "HouseInfoHeading", grp, CT_LABEL )
	...
	pc = c
	c = WINDOW_MANAGER:CreateControl( nil, grp, CT_TEXTURE )
	c:SetHeight( 5 )
	c:SetAlpha( 0.5 )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "ToggledItemsSection", parent, CT_CONTROL )
	ui.ToggledItemsSection = grp
	grp:SetAnchor( TOPLEFT, ui.HouseInfoSection, BOTTOMLEFT, 0, 20 )
	grp:SetResizeToFitDescendents( true )

	c = WINDOW_MANAGER:CreateControl( prefix .. "InteractiveItemsHeading", grp, CT_LABEL )
	...
	grp = WINDOW_MANAGER:CreateControl( prefix .. "TargetDummiesSection", parent, CT_CONTROL )
	ui.TargetDummiesSection = grp
	grp:SetAnchor( TOPLEFT, parent, TOP, 0, 5 )
	grp:SetResizeToFitDescendents( true )

	c = WINDOW_MANAGER:CreateControl( prefix .. "TargetDummiesHeading", grp, CT_LABEL )
	ui.TargetDummiesHeading = c
	c:SetFont( "ZoFontWinH5" )
	c:SetColor( 1, 1, 0.75, 1 )
	c:SetAnchor( TOPLEFT, grp, TOPLEFT, 5, 0 )
	c:SetAnchor( TOPRIGHT, grp, TOPRIGHT, 0, 0 )
	c:SetText( "Target Dummies" )
	...
	pc = c
	c = EHT.UI.CreateButton(
		prefix .. "ResetTargetDummies",
		grp,
		"Reset all dummies",
		{ { TOPLEFT, pc, BOTTOMLEFT, 5, 2 } },
		function() EHT.Biz.ResetTargetDummies() end )
	ui.ResetTargetDummies = c
	tip( c, "Resets all target dummies to full health.\n\nNote: You may also type |cffff33/resetdummy|r" )


	grp = WINDOW_MANAGER:CreateControl( prefix .. "ReportsSection", parent, CT_CONTROL )
	ui.ReportsSection = grp
	grp:SetAnchor( TOPLEFT, ui.TargetDummiesSection, BOTTOMLEFT, 0, 20 )
	grp:SetResizeToFitDescendents( true )

	c = WINDOW_MANAGER:CreateControl( prefix .. "ReportsHeading", grp, CT_LABEL )
	ui.ReportsHeading = c
	c:SetFont( "ZoFontWinH5" )
	c:SetColor( 1, 1, 0.75, 1 )
	c:SetAnchor( TOPLEFT, grp, TOPLEFT, 5, 0 )
...
function EHT.UI.SetupToolDialogHistoryTab( ui, parent, prefix, settings )
	...
	ui.HistoryListing = WINDOW_MANAGER:CreateControl( prefix .. "HistoryListing", parent, CT_CONTROL )
	ui.HistoryListing:SetAnchor( TOPLEFT, parent, TOPLEFT, 0, 0 )
	ui.HistoryListing:SetAnchor( BOTTOMRIGHT, parent, BOTTOMRIGHT, 0, -30 )

	-- History List

	ui.HistoryBuffer = WINDOW_MANAGER:CreateControl( prefix .. "HistoryBuffer", ui.HistoryListing, CT_TEXTBUFFER )
	...
	ui.HistorySlider = WINDOW_MANAGER:CreateControl( prefix .. "HistorySlider", ui.HistoryListing, CT_SLIDER )
	...
	ui.HistoryActions = WINDOW_MANAGER:CreateControl( prefix .. "HistoryActions", parent, CT_CONTROL )
	...
	local divider = WINDOW_MANAGER:CreateControl( nil, ui.HistoryActions, CT_TEXTURE )
	divider:SetHeight( 5 )
	divider:SetAlpha( 0.5 )
...
function EHT.UI.SetupToolDialog()
	...
	if nil == ui then
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "PersistentNotificationPanel", ui.Window, CT_CONTROL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "PersistentNotificationBackdrop", grp, CT_TEXTURE )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "PersistentNotificationLabel", grp, CT_LABEL )
		...
		ui.EditSection = WINDOW_MANAGER:CreateControl( prefix .. "EditSection", ui.Window, CT_CONTROL )
		...
		ui.TabControl = WINDOW_MANAGER:CreateControl( prefix .. "TabControl", ui.Window, CT_CONTROL )
		...
		ui.Tabs = WINDOW_MANAGER:CreateControl( prefix .. "Tabs", ui.TabControl, CT_CONTROL )
		...
		EHT.UI.SetInfoTooltip( ui.ToolTabButton, "One-click Toggle all lights, reset target dummies, publish a home's name and more." )
		...
		ui.NotificationPanel = WINDOW_MANAGER:CreateControl( prefix .. "NotificationPanel", ui.TabControl, CT_CONTROL )
		...
		ui.NotificationLabel = WINDOW_MANAGER:CreateControl( prefix .. "NotificationLabel", ui.NotificationPanel, CT_LABEL )
		...
		ui.TabContainer = WINDOW_MANAGER:CreateControl( prefix .. "TabContainer", ui.TabControl, CT_CONTROL )
		...
		ui.SelectionTab = WINDOW_MANAGER:CreateControl( prefix .. "SelectionTab", ui.TabContainer, CT_BUTTON )
		...
		ui.AnimateTab = WINDOW_MANAGER:CreateControl( prefix .. "AnimateTab", ui.TabContainer, CT_BUTTON )
		...
		ui.TriggerTab = WINDOW_MANAGER:CreateControl( prefix .. "TriggerTab", ui.TabContainer, CT_BUTTON )
		...
		ui.ToolTab = WINDOW_MANAGER:CreateControl( prefix .. "ToolTab", ui.TabContainer, CT_BUTTON )
		...
		ui.BuildTab = WINDOW_MANAGER:CreateControl( prefix .. "BuildTab", ui.TabContainer, CT_BUTTON )
		...
		ui.HistoryTab = WINDOW_MANAGER:CreateControl( prefix .. "HistoryTab", ui.TabContainer, CT_BUTTON )
		...
		do
			local container
			do
				local c = WINDOW_MANAGER:CreateControl(prefix .. "ProcessProgressBarContainer", ui.Window, CT_CONTROL)
				container = c
			...
			do
				local c = WINDOW_MANAGER:CreateControl(prefix .. "ProcessProgressBarBkg", container, CT_TEXTURE)
			...
			do
				local c = WINDOW_MANAGER:CreateControl(prefix .. "ProcessProgressBar", ui.ProcessProgressBarBkg, CT_TEXTURE)
			...
			do
				local c = WINDOW_MANAGER:CreateControl(prefix .. "ProcessName", container, CT_LABEL)
				ui.ProcessName = c
				c:SetFont("ZoFontWinH5")
				c:SetAnchor(BOTTOM)
				c:SetColor(1, 1, 1, 1)
		...
		do
			local b = WINDOW_MANAGER:CreateControl( prefix .. "SettingsButton", ui.Window, CT_BUTTON )
			ui.SettingsButton = b
		...
		do
			local b = WINDOW_MANAGER:CreateControl( prefix .. "FeedbackButton", ui.Window, CT_BUTTON )
			ui.FeedbackButton = b
		...
		do
			local b = WINDOW_MANAGER:CreateControl( prefix .. "HelpButton", ui.Window, CT_BUTTON )
			ui.HelpButton = b
			b:SetDimensions( 16, 16 )
			b:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
		...
		do
			local b = WINDOW_MANAGER:CreateControl( prefix .. "VideoGuideButton", ui.Window, CT_BUTTON )
...
do
	...
	local function AnimateEHTButtonBackdrop()
		...
		local duration, maxWeight = 1 == target and 460 or 600, 1 == target and 16 or 24
	...
	function EHT.UI.SetupEHTButton()
		...
		if nil == ui then
			do
				...
				local c = WINDOW_MANAGER:CreateControl( "EHTHouseOwner", win, CT_LABEL )
				...
				local c = WINDOW_MANAGER:CreateControl( "EHTHouseName", win, CT_LABEL )
				...
				c:SetHorizontalAlignment( TEXT_ALIGN_RIGHT )
				c:SetText( "" )
				c:SetAnchor( TOPRIGHT, EHTHouseOwner, BOTTOMRIGHT, 0, 0 )
				c:SetMouseEnabled( false )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "Caption", win, CT_LABEL )
			ui.Caption = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "AltCaption", win, CT_LABEL )
			ui.AltCaption = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "SummonStorage", win, CT_TEXTURE )
			ui.SummonStorage = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "HousingTools", win, CT_TEXTURE )
			ui.HousingTools = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "HousingHub", win, CT_TEXTURE )
			ui.HousingHub = c
			c:SetTexture( EHT.Textures.ICON_RADIAL_MENU )
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			c:SetDimensions( 28, 28 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "MailOwner", win, CT_TEXTURE )
			ui.MailOwner = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "Options", win, CT_TEXTURE )
			ui.Options = c
			c:SetTexture( EHT.Textures.ICON_RADIAL_MENU )
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			c:SetDimensions( 28, 28 )
			...
			local limits = WINDOW_MANAGER:CreateControl( prefix .. "LimitContainer", win, CT_CONTROL )
			...
			grp = WINDOW_MANAGER:CreateControl( prefix .. "PopLimit", limits, CT_CONTROL )
			ui.PopLimit = grp
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PopulationIcon", grp, CT_TEXTURE )
			ui.PopulationIcon = c
			c:SetTexture( EHT.Textures.ICON_LIMITS )
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PopulationLabel", grp, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PopulationMaxLabel", grp, CT_LABEL )
			...
			grp = WINDOW_MANAGER:CreateControl( prefix .. "FurnitureLimit", limits, CT_CONTROL )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "FurnitureIcon", grp, CT_TEXTURE )
			ui.FurnitureIcon = c
			c:SetTexture( EHT.Textures.ICON_LIMITS )
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			c:SetDimensions( 23, 25 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "LimitLabel", grp, CT_LABEL )
			ui.LimitLabel = c
			c:SetFont( "$(BOLD_FONT)|$(KB_18)|thick-outline" )
			c:SetColor(UnpackColor(Colors.Label, 0.9))
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "LimitMaxLabel", grp, CT_LABEL )
			ui.LimitMaxLabel = c
			c:SetFont( "$(MEDIUM_FONT)|$(KB_15)|outline" )
			c:SetColor(UnpackColor(Colors.Label, 0.9))
			...
			grp = WINDOW_MANAGER:CreateControl( prefix .. "FXLimit", limits, CT_CONTROL )
			ui.FXLimit = grp
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "FXIcon", grp, CT_TEXTURE )
			ui.FXIcon = c
			c:SetTexture( EHT.Textures.ICON_LIMITS )
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			c:SetDimensions( 26, 26 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "FXLabel", grp, CT_LABEL )
			ui.FXLabel = c
			c:SetFont( "$(BOLD_FONT)|$(KB_18)|thick-outline" )
			c:SetColor(UnpackColor(Colors.Label, 0.9))
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "FXMaxLabel", grp, CT_LABEL )
			ui.FXMaxLabel = c
			c:SetFont( "$(MEDIUM_FONT)|$(KB_15)|outline" )
			c:SetColor(UnpackColor(Colors.Label, 0.9))
...
function EHT.UI.SetupCertificationDialog()
	...
	if nil == ui then
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "EHTLabel", win, CT_LABEL )
		ui.EHTLabel = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "CertLabel", win, CT_LABEL )
		ui.CertLabel = c
...
do
	...
	function EHT.UI.SetupDeathCounterDialog()
		...
		if nil == ui then
			...
			c = WINDOW_MANAGER:CreateControl( nil, w, CT_LABEL )
			ui.Sponsor = c
...
function EHT.UI.SetupManageSelectionsDialog()
	...
	if nil == ui then
		...
		ui.LoadSelectionGroup = WINDOW_MANAGER:CreateControl( prefix .. "LoadSelectionGroup", ui.Window, CT_CONTROL )
		...
		ui.LoadSelectionLabel = WINDOW_MANAGER:CreateControl( prefix .. "LoadSelectionLabel", ui.LoadSelectionGroup, CT_LABEL )
		...
		ui.Divider1 = WINDOW_MANAGER:CreateControl( prefix .. "Divider1", ui.Window, CT_TEXTURE )
		...
		ui.SaveSelectionGroup = WINDOW_MANAGER:CreateControl( prefix .. "SaveSelectionGroup", ui.Window, CT_CONTROL )
		...
		ui.SaveSelectionLabel = WINDOW_MANAGER:CreateControl( prefix .. "SaveSelectionLabel", ui.SaveSelectionGroup, CT_LABEL )
		...
		ui.CloseButton = WINDOW_MANAGER:CreateControl( prefix .. "CloseButton", ui.Window, CT_BUTTON )
...
function EHT.UI.SetupManageBuildsDialog()
	...
	if nil == ui then
		...
		ui.LoadBuildGroup = WINDOW_MANAGER:CreateControl( prefix .. "LoadBuildGroup", ui.Window, CT_CONTROL )
		...
		ui.LoadBuildLabel = WINDOW_MANAGER:CreateControl( prefix .. "LoadBuildLabel", ui.LoadBuildGroup, CT_LABEL )
		...
		local div = WINDOW_MANAGER:CreateControl( nil, ui.Window, CT_TEXTURE )
		div:SetHeight( 5 )
		...
		ui.SaveBuildGroup = WINDOW_MANAGER:CreateControl( prefix .. "SaveBuildGroup", ui.Window, CT_CONTROL )
		...
		ui.SaveBuildLabel = WINDOW_MANAGER:CreateControl( prefix .. "SaveBuildLabel", ui.SaveBuildGroup, CT_LABEL )
...
function EHT.UI.SetupClipboardDialog()
	...
	if nil == ui then
		...
		ctl = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardLabel", ui.Window, CT_LABEL )
		...
		ui.ClipboardGroup = WINDOW_MANAGER:CreateControl( prefix .. "ClipboardGroup", ui.Window, CT_CONTROL )
...
function EHT.UI.SetupImportClipboardDialog()
	...
	if nil == ui then
		...
		ui.Content = WINDOW_MANAGER:CreateControl( prefix .. "Content", ui.Window, CT_CONTROL )
		ui.Content:SetAnchor( TOPLEFT, nil, nil, 15, 15 )
		ui.Content:SetAnchor( BOTTOMRIGHT, nil, nil, -15, -15 )
		ui.Content:SetMouseEnabled( false )

		ui.Caption = WINDOW_MANAGER:CreateControl( prefix .. "Caption", ui.Content, CT_LABEL )
		...
		ui.Instructions = WINDOW_MANAGER:CreateControl( prefix .. "Instructions", ui.Content, CT_LABEL )
		...
		ui.Message = WINDOW_MANAGER:CreateControl( prefix .. "Message", ui.Content, CT_LABEL )
...
function EHT.UI.SetupCopyFromSelectionsDialog()
	...
	if nil == ui then
		...
		ui.SelectionGroup = WINDOW_MANAGER:CreateControl( prefix .. "SelectionGroup", ui.Window, CT_CONTROL )
		...
		ui.HouseLabel = WINDOW_MANAGER:CreateControl( prefix .. "HouseLabel", ui.SelectionGroup, CT_LABEL )
		...
		ui.SelectionLabel = WINDOW_MANAGER:CreateControl( prefix .. "SelectionLabel", ui.SelectionGroup, CT_LABEL )
...
function EHT.UI.SetupAddSelectionDialog()
	...
	if nil == ui then
		...
		ui.SelectionGroup = WINDOW_MANAGER:CreateControl( nil, ui.Window, CT_CONTROL )
...
function EHT.UI.SetupCloneSceneDialog()
	...
	if nil == ui then
		...
		ui.SceneGroup = WINDOW_MANAGER:CreateControl( prefix .. "SceneGroup", ui.Window, CT_CONTROL )
		...
		ui.HouseLabel = WINDOW_MANAGER:CreateControl( prefix .. "HouseLabel", ui.SceneGroup, CT_LABEL )
		ui.HouseLabel:SetText( "House" )
		ui.HouseLabel:SetFont( "ZoFontGameLarge" )
		ui.HouseLabel:SetHorizontalAlignment( TEXT_ALIGN_LEFT )
		...
		ui.SceneLabel = WINDOW_MANAGER:CreateControl( prefix .. "SceneLabel", ui.SceneGroup, CT_LABEL )
...
function EHT.UI.SetupBackupsDialog()
	...
	if nil == ui then
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupsTitle", win, CT_LABEL )
		ui.BackupsTitle = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupsStatus", win, CT_LABEL )
		ui.BackupsStatus = c
		c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
		c:SetColor( 1, 1, 1, 1 )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupsEnabled", win, CT_LABEL )
		ui.BackupsEnabled = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupsDisabled", win, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupSettings", win, CT_LABEL )
		ui.BackupSettings = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupsInfo", win, CT_LABEL )
		ui.BackupsInfo = c
		c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "RestoreStatusLabel", win, CT_LABEL )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "BackupsGroup", win, CT_CONTROL )
		ui.BackupsGroup = grp
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "BackupListLabel", grp, CT_LABEL )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "CloseButton", win, CT_BUTTON )
		ui.CloseButton = c
...
function EHT.UI.SetupPositionDialog()
	...
	if nil == ui then
		...
		ui.Backdrop = WINDOW_MANAGER:CreateControl( prefix .. "Backdrop", ui.Window, CT_TEXTURE )
		...
		ui.Top = WINDOW_MANAGER:CreateControl( prefix .. "Top", ui.Window, CT_CONTROL )
		...
		ui.TopBackdrop = WINDOW_MANAGER:CreateControl( prefix .. "TopBackdrop", ui.Window, CT_TEXTURE )
		...
		ui.ItemLabel = WINDOW_MANAGER:CreateControl( prefix .. "ItemLabel", ui.Top, CT_LABEL )
		...
		ui.ItemIconBackdrop = WINDOW_MANAGER:CreateControl( prefix .. "ItemIconBackdrop", ui.Top, CT_TEXTURE )
		...
		ui.ItemIcon = WINDOW_MANAGER:CreateControl( prefix .. "ItemIcon", ui.Top, CT_TEXTURE )
		...
		ui.Bottom = WINDOW_MANAGER:CreateControl( prefix .. "Bottom", ui.Window, CT_CONTROL )
		...
		ui.MoveGroupItemsLabel = WINDOW_MANAGER:CreateControl( prefix .. "MoveGroupItemsLabel", ui.Bottom, CT_LABEL )
		...
		ui.ControlGroup = WINDOW_MANAGER:CreateControl( prefix .. "ControlGroup", ui.Window, CT_CONTROL )
		...
		ui.PositionGroup = WINDOW_MANAGER:CreateControl( prefix .. "PositionGroup", ui.ControlGroup, CT_CONTROL )
		ui.PositionGroup:SetAnchor( TOPLEFT, ui.ControlGroup, TOPLEFT, 0, 0 )
		ui.PositionGroup:SetResizeToFitDescendents( true )


		ui.XLabel = WINDOW_MANAGER:CreateControl( prefix .. "XLabel", ui.PositionGroup, CT_LABEL )
		ui.XLabel:SetText( "X" )
		...
		ui.YLabel = WINDOW_MANAGER:CreateControl( prefix .. "YLabel", ui.PositionGroup, CT_LABEL )
		ui.YLabel:SetText( "Y" )
		...
		ui.ZLabel = WINDOW_MANAGER:CreateControl( prefix .. "ZLabel", ui.PositionGroup, CT_LABEL )
		ui.ZLabel:SetText( "Z" )
		...
		ui.OrientationGroup = WINDOW_MANAGER:CreateControl( prefix .. "OrientationGroup", ui.ControlGroup, CT_CONTROL )
		ui.OrientationGroup:SetAnchor( LEFT, ui.PositionGroup, RIGHT, 16, 0 )
		ui.OrientationGroup:SetResizeToFitDescendents( true )


		ui.PitchLabel = WINDOW_MANAGER:CreateControl( prefix .. "PitchLabel", ui.OrientationGroup, CT_LABEL )
		...
		ui.YawLabel = WINDOW_MANAGER:CreateControl( prefix .. "YawLabel", ui.OrientationGroup, CT_LABEL )
		...
		ui.RollLabel = WINDOW_MANAGER:CreateControl( prefix .. "RollLabel", ui.OrientationGroup, CT_LABEL )
		...
		ui.FXGroup = WINDOW_MANAGER:CreateControl( prefix .. "FXGroup", ui.ControlGroup, CT_CONTROL )
		ui.FXGroup:SetAnchor( LEFT, ui.OrientationGroup, RIGHT, 16, 0 )
		ui.FXGroup:SetResizeToFitDescendents( true )

		ui.ColorLabel = WINDOW_MANAGER:CreateControl( prefix .. "ColorLabel", ui.FXGroup, CT_LABEL )
		...
		ui.SizeLabel = WINDOW_MANAGER:CreateControl( prefix .. "SizeLabel", ui.FXGroup, CT_LABEL )
		...
		ui.CloseButton = WINDOW_MANAGER:CreateControl( prefix .. "CloseButton", ui.Window, CT_LABEL )
...
function EHT.UI.SetupEHTButtonContextMenu( anchorPoint, anchorControl, anchorControlPoint, anchorOffsetX, anchorOffsetY )
	...
	if nil == ui then
		...
		c = WINDOW_MANAGER:CreateControl( nil, win, CT_LINE )
		ui.Backdrop = c
		c:SetInheritAlpha( false )
...
--[[
			ui.BackdropShadow:SetVertexColors( 1 + 4, 0, 0.3, 0.4, 0.5 * alpha )
			ui.BackdropShadow:SetVertexColors( 2 + 8, 0.1, 0.1, 0.1, 0 )

			ui.Backdrop:SetVertexColors( 1 + 4, 0.4, 0.3, 0, 0.5 * alpha )
			ui.Backdrop:SetVertexColors( 2 + 8, 0.1, 0.1, 0.1, 0 )

			ui.Separator:SetVertexColors( 1 + 4, 1, 1, 1, 0.5 * alpha )
			ui.Separator:SetVertexColors( 2 + 8, 0, 0, 0, 0.25 * alpha )
]]
			local r, g, b = UnpackColor(Colors.Icon)
			ui.Backdrop:SetVertexColors( 1 + 4, r, g, b, 0.5 * alpha )
			ui.Backdrop:SetVertexColors( 2 + 8, 0.1, 0.1, 0.1, 0 )
		...
		local pl = WINDOW_MANAGER:CreateControl( prefix .. "PanelLeft", win, CT_CONTROL )
		ui[ "PanelLeft" ] = pl
		...
		local pr = WINDOW_MANAGER:CreateControl( prefix .. "PanelRight", win, CT_CONTROL )
		ui[ "PanelRight" ] = pr
		...
		local function CreateSection( ui, window, anchorControl, anchorPosition, name, label )
			local sect = WINDOW_MANAGER:CreateControl( prefix .. name .. "Section", window, CT_CONTROL )
			...
			local lbl = WINDOW_MANAGER:CreateControl( prefix .. name .. "Label", sect, CT_LABEL )
			ui[ name .. "Label" ] = lbl
		...
		local function CreateField( ui, container, textureFile, name, label, tooltip, onClick )
			...
			local grp = WINDOW_MANAGER:CreateControl( prefix .. name .. "Group", container, CT_CONTROL )
			...
			if showIcon then
				local c = WINDOW_MANAGER:CreateControl( prefix .. name, grp, CT_TEXTURE )
				ui[ name ] = c
			...
			local lbl = WINDOW_MANAGER:CreateControl( prefix .. name .. "Label", grp, CT_LABEL )
			ui[ name .. "Label" ] = lbl
			lbl:SetDrawLevel( baseDrawLevel )
		...
		local lbTools = WINDOW_MANAGER:CreateControl( prefix .. "ToolsLabel", pl, CT_LABEL )
		...
		local lbToggles = WINDOW_MANAGER:CreateControl( prefix .. "TogglesLabel", pl, CT_LABEL )
...
do
	...
	local function CreateSection( label, level, ui, parent, anchorControl, anchorPosition, anchorOffsetX, anchorOffsetY )
		...
		local sect = WINDOW_MANAGER:CreateControl( nil, parent, CT_CONTROL )
	...
	local function CreateField( ui, container, label, onClick, bold, recordId, onMouseEnter, onMouseExit )
		local bottomControl = container.BottomControl
		local lbl = WINDOW_MANAGER:CreateControl( nil, container, CT_LABEL )
	...
	function EHT.UI.SetupEHTEffectsButtonContextMenu( anchorPoint, anchorControl, anchorControlPoint, anchorOffsetX, anchorOffsetY )
		...
		if nil == ui then
			...
			do
				local lbl = WINDOW_MANAGER:CreateControl( prefix .. "Heading", win, CT_LABEL )
				ui.Heading = lbl
				lbl:SetDrawLevel( baseDrawLevel )
				...
				lbl = WINDOW_MANAGER:CreateControl( prefix .. "HeadingTM", win, CT_LABEL )
				ui.HeadingTM = lbl
				lbl:SetDrawLevel( baseDrawLevel )
			...
			do
				...
				c = WINDOW_MANAGER:CreateControl( nil, windowFrame, CT_LABEL )
				ui.NumEffects = c
				c:SetDrawLevel( baseDrawLevel )
			...
			do
				local bkd = WINDOW_MANAGER:CreateControl( nil, windowFrame, CT_TEXTURE )
				...
				local btn = WINDOW_MANAGER:CreateControl( prefix .. "AddEffects", windowFrame, CT_LABEL )
				ui.AddEffects = btn
			...
			do
				...
				local btn = WINDOW_MANAGER:CreateControl( prefix .. "PlacedEffects", windowFrame, CT_LABEL )
			...
			do
				...
				local btn = WINDOW_MANAGER:CreateControl( prefix .. "ShareEffects", windowFrame, CT_LABEL )
				ui.ShareEffects = btn
				btn:SetDrawLevel( baseDrawLevel )
				btn:SetFont( "$(BOLD_FONT)|$(KB_18)|outline" )
				btn:SetAnchor( CENTER, bkd, CENTER, 0, 0 )
			...
			do
				local lbl = WINDOW_MANAGER:CreateControl( prefix .. "ShowHideEffectsLabel", windowFrame, CT_LABEL )
			...
			do
				local lbl = WINDOW_MANAGER:CreateControl( prefix .. "RemoveAllEffectsLabel", windowFrame, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, shareControls, CT_CONTROL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, publishedControls, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, publishedControls, CT_TEXTURE )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.RevertToOriginalOutline, CT_TEXTURE )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.RevertToOriginal, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.AcceptPreviewOutline, CT_TEXTURE )
			ui.AcceptPreview = c
			c:SetDrawLevel( baseDrawLevel + 1 )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.AcceptPreview, CT_LABEL )
			...
			do
				local lbl = WINDOW_MANAGER:CreateControl( prefix .. "ShareEffectsLabel", shareControls, CT_LABEL )
				...
				local lbl = WINDOW_MANAGER:CreateControl( prefix .. "ShareEffectsTip", bkg, CT_LABEL )
				lbl:SetDrawLevel( baseDrawLevel )
			...
			c = WINDOW_MANAGER:CreateControl( nil, shareControls, CT_TEXTURE )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.PublishButtonOutline, CT_TEXTURE )
			ui.PublishButton = c
			c:SetDrawLevel( baseDrawLevel + 1 )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.PublishButton, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, shareControls, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.GuildcastButtonOutline, CT_TEXTURE )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.GuildcastButton, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.ChatcastButtonOutline, CT_TEXTURE )
			ui.ChatcastButton = c
			c:SetDrawLevel( baseDrawLevel + 1 )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.ChatcastButton, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.MailcastButtonOutline, CT_TEXTURE )
			ui.MailcastButton = c
			c:SetDrawLevel( baseDrawLevel + 1 )
			...
			c = WINDOW_MANAGER:CreateControl( nil, ui.MailcastButton, CT_LABEL )
...
function EHT.UI.SetupSnapFurnitureDialog()
	...
	if nil == ui then
		...
		ui.ItemLabelBackdrop = WINDOW_MANAGER:CreateControl( prefix .. "ItemLabelBackdrop", win, CT_LINE )
		...
		ui.Backdrop = WINDOW_MANAGER:CreateControl( prefix .. "Backdrop", win, CT_LINE )
		ctl = ui.Backdrop
		...
		ui.ItemLabel = WINDOW_MANAGER:CreateControl( prefix .. "ItemLabel", win, CT_LABEL )
		ctl = ui.ItemLabel
		...
		ui.XMarginGroup = WINDOW_MANAGER:CreateControl( prefix .. "XMarginGroup", win, CT_CONTROL )
		grp = ui.XMarginGroup
		grp:SetAnchor( LEFT, win, LEFT, 5, -28 )
		grp:SetDimensions( 105, 94 )

		ui.XMarginDec = WINDOW_MANAGER:CreateControl( prefix .. "XMarginDec", grp, CT_BUTTON )
		ctl = ui.XMarginDec
		...
		ui.XMarginInc = WINDOW_MANAGER:CreateControl( prefix .. "XMarginInc", grp, CT_BUTTON )
		ctl = ui.XMarginInc
		ctl:SetDimensions( 40, 40 )
		...
		ctl = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
		ctl:SetColor( 1, 1, 1, 1 )
		...
		ui.XMarginValue = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
		ctl = ui.XMarginValue
		ctl:SetColor( 1, 1, 1, 1 )
		ctl:SetFont( "$(MEDIUM_FONT)|$(KB_18)|soft-shadow-thick" )
		ctl:SetText( "0 cm" )
		...
		ui.YMarginGroup = WINDOW_MANAGER:CreateControl( prefix .. "YMarginGroup", win, CT_CONTROL )
		grp = ui.YMarginGroup
		grp:SetAnchor( CENTER, win, CENTER, 0, -28 )
		grp:SetDimensions( 105, 94 )

		ui.YMarginDec = WINDOW_MANAGER:CreateControl( prefix .. "YMarginDec", grp, CT_BUTTON )
		ctl = ui.YMarginDec
		ctl:SetDimensions( 40, 40 )
		ctl:SetAnchor( LEFT, grp, LEFT, 5, 0 )
		...
		ui.YMarginInc = WINDOW_MANAGER:CreateControl( prefix .. "YMarginInc", grp, CT_BUTTON )
		ctl = ui.YMarginInc
		ctl:SetDimensions( 40, 40 )
		ctl:SetAnchor( RIGHT, grp, RIGHT, -5, 0 )
		...
		ui.YMarginValue = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
		ctl = ui.YMarginValue
		ctl:SetColor( 1, 1, 1, 1 )
		ctl:SetFont( "$(MEDIUM_FONT)|$(KB_18)|soft-shadow-thick" )
		ctl:SetText( "0 cm" )
		...
		ui.ZMarginGroup = WINDOW_MANAGER:CreateControl( prefix .. "ZMarginGroup", win, CT_CONTROL )
		grp = ui.ZMarginGroup
		grp:SetAnchor( RIGHT, win, RIGHT, -5, -28 )
		grp:SetDimensions( 105, 94 )

		ui.ZMarginDec = WINDOW_MANAGER:CreateControl( prefix .. "ZMarginDec", grp, CT_BUTTON )
		ctl = ui.ZMarginDec
		ctl:SetDimensions( 40, 40 )
		ctl:SetAnchor( LEFT, grp, LEFT, 5, 0 )
		ctl:SetNormalTexture( "esoui/art/housing/housing_axiscontrolicon_yawcw.dds" )
		...
		ui.ZMarginInc = WINDOW_MANAGER:CreateControl( prefix .. "ZMarginInc", grp, CT_BUTTON )
		ctl = ui.ZMarginInc
		ctl:SetDimensions( 40, 40 )
		ctl:SetAnchor( RIGHT, grp, RIGHT, -5, 0 )
		ctl:SetNormalTexture( "esoui/art/housing/housing_axiscontrolicon_yawccw.dds" )
		...
		ui.ZMarginValue = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
		ctl = ui.ZMarginValue
		ctl:SetColor( 1, 1, 1, 1 )
		ctl:SetFont( "$(MEDIUM_FONT)|$(KB_18)|soft-shadow-thick" )
		ctl:SetText( "0 cm" )
		...
		ui.SnapItemGroup = WINDOW_MANAGER:CreateControl( prefix .. "SnapItemGroup", win, CT_CONTROL )
		grp = ui.SnapItemGroup
		grp:SetAnchor( BOTTOM, win, BOTTOM, -60, -5 )
		grp:SetDimensions( 105, 94 )

		ui.SnapPrevItem = WINDOW_MANAGER:CreateControl( prefix .. "SnapPrevItem", grp, CT_BUTTON )
		ctl = ui.SnapPrevItem
		ctl:SetDimensions( 40, 40 )
		...
		ui.SnapNextItem = WINDOW_MANAGER:CreateControl( prefix .. "SnapNextItem", grp, CT_BUTTON )
		ctl = ui.SnapNextItem
		ctl:SetDimensions( 40, 40 )
		...
		ui.SnapItemCount = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
		...
		ui.OrientationGroup = WINDOW_MANAGER:CreateControl( prefix .. "OrientationGroup", win, CT_CONTROL )
		grp = ui.OrientationGroup
		grp:SetAnchor( BOTTOM, win, BOTTOM, 60, -5 )
		grp:SetDimensions( 105, 94 )

		ui.SnapPrevOrientation = WINDOW_MANAGER:CreateControl( prefix .. "SnapPrevOrientation", grp, CT_BUTTON )
		...
		ui.SnapNextOrientation = WINDOW_MANAGER:CreateControl( prefix .. "SnapNextOrientation", grp, CT_BUTTON )
		...
		ui.SnapOrientationCount = WINDOW_MANAGER:CreateControl( nil, grp, CT_LABEL )
		...
		ui.CloseButton = WINDOW_MANAGER:CreateControl( prefix .. "CloseButton", win, CT_BUTTON )
		ctl = ui.CloseButton
...
function EHT.UI.SetupEditFurnitureDialog()
	...
	if nil == ui then
		...
		ctl = WINDOW_MANAGER:CreateControl( prefix .. "HeaderBackdrop", win, CT_LINE )
		...
		ctl = WINDOW_MANAGER:CreateControl( prefix .. "Backdrop", win, CT_LINE )
		ui.Backdrop = ctl
		...
		ctl = WINDOW_MANAGER:CreateControl( prefix .. "ItemLabel", win, CT_LABEL )
		ui.ItemLabel = ctl
		ctl:SetColor( 1, 1, 1, 1 )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "EditControls", win, CT_CONTROL )
		ui.EditControls = grp
...
function EHT.UI.SetupTutorialDialog()
	...
	if nil == ui then
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "Title", win, CT_LABEL )
		ui.Title = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "UpArrow", win, CT_TEXTURE )
		ui.UpArrow = c
		c:SetHidden( true )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "DownArrow", win, CT_TEXTURE )
		ui.DownArrow = c
		c:SetHidden( true )
		c:SetTexture( EHT.Textures.ARROW_2 )
		c:SetDimensions( 64, 64 )
...
do
	...
	function EHT.UI.SetupItemInfoDialog()
		...
		if nil == ui then
			...
			grp = WINDOW_MANAGER:CreateControl( prefix .. "ItemInfoGroup", win, CT_CONTROL )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemIcon", win, CT_TEXTURE )
			ui.ItemIcon = c
			c:SetColor( 1, 1, 1, 1 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemLabel", grp, CT_LABEL )
			ui.ItemLabel = c
			c:SetColor( 1, 1, 1, 1 )
			c:SetFont( "$(BOLD_FONT)|$(KB_20)" )
			c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemLimit", grp, CT_LABEL )
			ui.ItemLimit = c
			c:SetColor( 1, 1, 1, 1 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemOrientation", grp, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemPosition", grp, CT_LABEL )
			ui.ItemPosition = c
			c:SetColor( 1, 1, 1, 1 )
			c:SetFont( "$(CHAT_FONT)|$(KB_16)" )
			c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemDimensions", grp, CT_LABEL )
			ui.ItemDimensions = c
			c:SetColor( 1, 1, 1, 1 )
			c:SetFont( "$(CHAT_FONT)|$(KB_16)" )
			c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ItemDecoTrack", grp, CT_LABEL )
			ui.ItemDecoTrack = c
			c:SetColor( 1, 1, 1, 1 )
			c:SetFont( "$(CHAT_FONT)|$(KB_16)" )
			c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
...
function EHT.UI.RefreshItemInfoDialog()
	...
	local dist = zo_distance3D( x, y, z, pX, pY, pZ ) / 100
	local limitName, limitMax, limitUsed = EHT.Housing.GetLimit( limitType )

	ui.ItemOrientation:SetText( string.format( "|rpitch: %s%.2f|r  yaw: %s%.2f|r  roll: %s%.2f|r", cData, math.deg( pitch or 0 ), cData, math.deg( yaw or 0 ), cData, math.deg( roll or 0 ) ) )
	ui.ItemPosition:SetText( string.format( "|rx: %s%d|rcm  y: %s%d|rcm  z: %s%d|rcm  (%s%.2f|rm away)", cData, x or 0, cData, y or 0, cData, z or 0, cData, dist ) )
	ui.ItemDimensions:SetText( string.format( "|rwidth: %s%.2f|rm  height: %s%.2f|rm  depth: %s%.2f|rm", cData, sizeX / 100, cData, sizeY / 100, cData, sizeZ / 100 ) )
...
------[[ HUD : House Information ]]------

function EHT.UI.SetupHouseInfoDialog()
	local ui = EHT.UI.HouseInfoDialog
	if nil == ui then
		ui = { }
		EHT.UI.HouseInfoDialog = ui

		local prefix = "EHTHouseInfoDialog"
		local c, grp, win
		...
		win:SetMovable( false )
		win:SetResizeHandleSize( 0 )
		win:SetAnchor( TOP, GuiRoot, TOP, 0, 340 )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "Subtitle", win, CT_LABEL )
		ui.Subtitle = c
		c:SetColor( 0.75, 0.75, 0.25, 1 )
		c:SetText( "" )
		c:SetFont( "$(BOLD_FONT)|$(KB_22)|soft-shadow-thick" )
		c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
		c:SetVerticalAlignment( TEXT_ALIGN_TOP )
		c:SetAnchor( TOP, ui.Title, BOTTOM, 0, -5 )
	end

	return ui
end

function EHT.UI.HideHouseInfo()
	local ui = EHT.UI.SetupHouseInfoDialog()
	ui.Window:SetHidden( true )
end

function EHT.UI.OnUpdateHouseInfo()
	local ui = EHT.UI.HouseInfoDialog
	local alpha = ui.Window:GetAlpha()
	local startMS = ui.AnimationStartMS
	local intervalMS = GetFrameTimeMilliseconds() - startMS
	local interval = intervalMS / ui.AnimationDurationMS

	if 1 < interval then
		ui.Window:SetAlpha(0)
		ui.Window:SetHidden( true )
		EVENT_MANAGER:UnregisterForUpdate( "EHT.UI.OnUpdateHouseInfo" )
		return
	end

	ui.Window:SetAlpha( EHT.World.VariableEase( interval, 0.5 ) )
end
...
function EHT.UI.OnHouseNameMoved()
	...
	EHTHouseOwner:ClearAnchors()
	EHTHouseOwner:SetAnchor( ownerAnchorLocal, win, ownerAnchorRelative, 0, 0 )
	EHTHouseOwner:SetHorizontalAlignment( textAlign )

	EHTHouseName:ClearAnchors()
	EHTHouseName:SetAnchor( nameAnchorLocal, EHTHouseOwner, nameAnchorRelative, 0, 0 )
...
function EHT.UI.SetCustomHouseName( name )
	...
	if EHTHouseNameWin then
		if isOwner then
			EHTHouseOwner:SetText( EHT.CurrentHouseName or houseName or "" )
			EHTHouseName:SetText( "" )
		else
			EHTHouseOwner:SetText( ownerName or "" )
			EHTHouseName:SetText( EHT.CurrentHouseName or houseName or "" )
		end
	end
end

function EHT.UI.DisplayHouseInfo()
	if not EHT.SavedVars.DisplayHouseInfoUponEntry then return end

	local houseId, owner, isOwner, houseName, houseNickname, customHouseNickname = EHT.Housing.GetHouseInfo()
	if 0 ~= houseId then
		local ui = EHT.UI.SetupHouseInfoDialog()

		if isOwner then
			EHT.UI.SetCustomHouseName( nil )
			ui.Title:SetText( EHT.UI.GetCustomHouseName() )
			ui.Subtitle:SetText( houseName )
		else
			EHT.UI.SetCustomHouseName( nil )
			ui.Title:SetText( EHT.UI.GetCustomHouseName() )

			if nil ~= owner and "" ~= owner then
				ui.Subtitle:SetText( string.format( "Property of %s", owner ) )
			else
				ui.Subtitle:SetText( "" )
			end
		end

		ui.AnimationStartMS = GetFrameTimeMilliseconds()
		ui.AnimationDurationMS = 5000
		ui.Window:SetAlpha( 0 )
		ui.Window:SetHidden( false )

		EVENT_MANAGER:RegisterForUpdate( "EHT.UI.OnUpdateHouseInfo", 1, EHT.UI.OnUpdateHouseInfo )
	end
end
...
function EHT.UI.SetupAdjustGuidelinesDialog()
	...
	if nil == ui then
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "MovementGroup", win, CT_CONTROL )
		ui.MovementGroup = grp
		grp:SetAnchor( CENTER, win, CENTER, 0, 0 )
		grp:SetDimensions( 300, 300 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "MoveEast", grp, CT_TEXTURE )
		ui.MoveEast = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "EastLabel", grp, CT_LABEL )
		ui.EastLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "E" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "MoveWest", grp, CT_TEXTURE )
		ui.MoveWest = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "WestLabel", grp, CT_LABEL )
		ui.WestLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "W" )
		c:SetFont( "$(CHAT_FONT)|$(KB_18)|thick-outline" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "MoveNorth", grp, CT_TEXTURE )
		ui.MoveNorth = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "NorthLabel", grp, CT_LABEL )
		ui.NorthLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "N" )
		c:SetFont( "$(CHAT_FONT)|$(KB_18)|thick-outline" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "MoveSouth", grp, CT_TEXTURE )
		ui.MoveSouth = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "SouthLabel", grp, CT_LABEL )
		ui.SouthLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "S" )
		c:SetFont( "$(CHAT_FONT)|$(KB_18)|thick-outline" )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "ScaleGroup", win, CT_CONTROL )
		ui.ScaleGroup = grp
		grp:SetAnchor( LEFT, win, LEFT, 0, 0 )
		grp:SetDimensions( 100, 300 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "ScaleUp", grp, CT_TEXTURE )
		ui.ScaleUp = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "ScaleUpLabel", grp, CT_LABEL )
		ui.ScaleUpLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "Larger" )
		c:SetFont( "$(CHAT_FONT)|$(KB_18)|thick-outline" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "ScaleDown", grp, CT_TEXTURE )
		ui.ScaleDown = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "ScaleDownLabel", grp, CT_LABEL )
		ui.ScaleDownLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "Smaller" )
		c:SetFont( "$(CHAT_FONT)|$(KB_18)|thick-outline" )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "RotateGroup", win, CT_CONTROL )
		ui.RotateGroup = grp
		grp:SetAnchor( RIGHT, win, RIGHT, 0, 0 )
		grp:SetDimensions( 100, 300 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "RotateLabel", grp, CT_LABEL )
		ui.RotateLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "Rotate" )
		c:SetFont( "$(CHAT_FONT)|$(KB_18)|thick-outline" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "RotateCW", grp, CT_TEXTURE )
		ui.RotateCW = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "RotateCCW", grp, CT_TEXTURE )
		ui.RotateCCW = c
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "CloseGroup", win, CT_CONTROL )
		ui.CloseGroup = grp
		grp:SetAnchor( TOP, win, BOTTOM, 0, 0 )
		grp:SetResizeToFitDescendents( true )

		c = WINDOW_MANAGER:CreateControl( prefix .. "CloseLabel", grp, CT_LABEL )
		ui.CloseLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "CLOSE" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "Close", grp, CT_TEXTURE )
		ui.Close = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "InstructionLabel", win, CT_LABEL )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "ScaleHeaderGroup", win, CT_CONTROL )
		ui.ScaleHeaderGroup = grp
		grp:SetAnchor( BOTTOM, ui.ScaleGroup, TOP, 0, 0 )
		grp:SetDimensions( 100, 80 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "Scale", grp, CT_LABEL )
		ui.Scale = c
		c:SetColor( 1, 1, 0, 1 )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "ScaleLabel", grp, CT_LABEL )
		ui.ScaleLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "Scale" )
		c:SetFont( "$(CHAT_FONT)|$(KB_26)|thick-outline" )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "OffsetsHeaderGroup", win, CT_CONTROL )
		ui.OffsetsHeaderGroup = grp
		grp:SetAnchor( BOTTOM, ui.MovementGroup, TOP, 0, 0 )
		grp:SetDimensions( 100, 80 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "Offsets", grp, CT_LABEL )
		ui.Offsets = c
		c:SetColor( 1, 1, 0, 1 )
		c:SetFont( "$(CHAT_FONT)|$(KB_32)|thick-outline" )
		c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "OffsetsLabel", grp, CT_LABEL )
		ui.OffsetsLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "Offsets" )
		c:SetFont( "$(CHAT_FONT)|$(KB_26)|thick-outline" )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "RotateHeaderGroup", win, CT_CONTROL )
		ui.RotateHeaderGroup = grp
		grp:SetAnchor( BOTTOM, ui.RotateGroup, TOP, 0, 0 )
		grp:SetDimensions( 100, 80 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "Rotation", grp, CT_LABEL )
		ui.Rotation = c
		c:SetColor( 1, 1, 0, 1 )
		c:SetFont( "$(CHAT_FONT)|$(KB_32)|thick-outline" )
		c:SetHorizontalAlignment( TEXT_ALIGN_CENTER )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "RotationLabel", grp, CT_LABEL )
		ui.RotationLabel = c
		c:SetColor( 1, 1, 1, 1 )
		c:SetText( "Rotation" )
		c:SetFont( "$(CHAT_FONT)|$(KB_26)|thick-outline" )
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "TitleGroup", win, CT_CONTROL )
		ui.TitleGroup = grp
		grp:SetAnchor( BOTTOM, ui.OffsetsHeaderGroup, TOP, 0, 0 )
		grp:SetDimensions( 200, 80 )

		c = WINDOW_MANAGER:CreateControl( prefix .. "TitleLabel", grp, CT_LABEL )
		ui.TitleLabel = c
		c:SetColor( 1, 1, 1, 1 )
...
function EHT.UI.SetupReportDialog()
	...
	if nil == ui then
		...
		local cgrp = WINDOW_MANAGER:CreateControl( prefix .. "Criteria", win, CT_CONTROL )
		ui.Criteria = cgrp
		...
		grp = WINDOW_MANAGER:CreateControl( prefix .. "HouseItemCriteria", cgrp, CT_CONTROL )
...
--[[
		...
		local grp = WINDOW_MANAGER:CreateControl( prefix .. "Paging", win, CT_CONTROL )
		ui.Paging = grp
		grp:SetAnchor( TOP, cgrp, BOTTOM, 0, 4 )
		grp:SetResizeToFitDescendents( true )

		c = WINDOW_MANAGER:CreateControl( prefix .. "PageCount", grp, CT_LABEL )
		ui.PageCount = c
		c:SetColor( 0.9, 0.9, 0.9, 1 )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "PageBack", grp, CT_LABEL )
		ui.PageBack = c
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "PageNext", grp, CT_LABEL )
		ui.PageNext = c
		c:SetColor( 0.9, 0.9, 0.4, 1 )
		c:SetText( "Next >>" )
		c:SetFont( "$(CHAT_FONT)|$(KB_15)|soft-shadow-thick" )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "Instructions", win, CT_LABEL )
		ui.Instructions = c
		c:SetColor( 0.9, 0.9, 0.9, 1 )
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "Close", win, CT_LABEL )
		ui.Close = c
...
function EHT.UI.SetupMapcastDialog()
	...
	if nil == ui then
		...
		c = WINDOW_MANAGER:CreateControl( prefix .. "Animation", win, CT_TEXTURE )
		ui.Animation = c
		c:SetAnchor( CENTER, win, CENTER, 0, 0 )
...
do
	...
	function EHT.UI.SetupPreviewControlsDialog()
		...
		if nil == ui then
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "Pad", win, CT_TEXTURE )
			ui.Pad = c
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			c:SetAnchor( RIGHT, win, RIGHT, 0, 0 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "RulerBackdrop", win, CT_TEXTURE )
			ui.RulerBackdrop = c
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "Ruler", win, CT_TEXTURE )
			ui.Ruler = c
			c:SetTextureReleaseOption( RELEASE_TEXTURE_AT_ZERO_REFERENCES )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "Pan", win, CT_LABEL )
			ui.Pan = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "Tilt", win, CT_LABEL )
			ui.Tilt = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "RulerLabel", win, CT_LABEL )
			ui.RulerLabel = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PadUp", win, CT_TEXTURE )
			ui.PadUp = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PadDown", win, CT_TEXTURE )
			ui.PadDown = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PadLeft", win, CT_TEXTURE )
			ui.PadLeft = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PadRight", win, CT_TEXTURE )
			ui.PadRight = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "PadRuler", win, CT_TEXTURE )
			ui.PadRuler = c
...
do
	...
	function EHT.UI.SetupTriggerQueue()
		...
		if not ui then
			...
			do
				grp = WINDOW_MANAGER:CreateControl( nil, win, CT_CONTROL )
				ui.TitleBar = grp
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "Heading", grp, CT_LABEL )
				ui.Heading = c
				c:SetDrawLevel( baseDrawLevel )
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "Count", grp, CT_LABEL )
				ui.Count = c
				c:SetDrawLevel( baseDrawLevel )
				c:SetFont( "$(BOLD_FONT)|$(KB_22)|soft-shadow-thick" )
				c:SetColor( 0, 0.9, 0.9, 1 )
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "CloseButton", grp, CT_TEXTURE )
				ui.CloseButton = c
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "CloseButtonLabel", grp, CT_LABEL )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ScrollPanel", windowFrame, CT_CONTROL )
			ui.ScrollPanel = c
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "ScrollList", ui.ScrollPanel, CT_CONTROL )
			ui.ScrollList = c
			local list = c
	...
	function EHT.UI.CreateTriggerQueueRow( index )
		...
		control = WINDOW_MANAGER:CreateControl( nil, ui.ScrollList, CT_CONTROL )
		...
		c = WINDOW_MANAGER:CreateControl( nil, control, CT_TEXTURE )
		...
		c = WINDOW_MANAGER:CreateControl( nil, control, CT_LABEL )
		control.Index = c
		c:SetDrawLevel( baseDrawLevel )
	...
	function EHT.UI.SetupGuildcast()
		...
		if not ui then
			...
			do
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "HeadingBackdrop", grp, CT_LABEL )
				ui.HeadingBackdrop = c
				c:SetDrawLevel( baseDrawLevel + 1 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "HouseNameLabel", windowFrame, CT_LABEL )
			ui.HouseNameLabel = c
			c:SetDrawLevel( baseDrawLevel )
			c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
			c:SetColor( 1, 1, 1, 1 )
			...
			c = WINDOW_MANAGER:CreateControl( prefix .. "HouseOwnerLabel", windowFrame, CT_LABEL )
			...
			do
				...
				c = WINDOW_MANAGER:CreateControl( nil, ui.ShareButtonOutline, CT_TEXTURE )
				ui.ShareButton = c
				c:SetDrawLevel( baseDrawLevel + 1 )
				...
				c = WINDOW_MANAGER:CreateControl( nil, ui.ShareButton, CT_LABEL )
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "CloseButton", ui.CloseButtonOutline, CT_TEXTURE )
				ui.CloseButton = c
				c:SetDrawLevel( baseDrawLevel + 1 )
				c:SetAnchor( CENTER )
				c:SetDimensions( 117, 35 )
				...
				c = WINDOW_MANAGER:CreateControl( prefix .. "CloseButtonLabel", ui.CloseButton, CT_LABEL )
				ui.CloseButtonLabel = c
				c:SetDrawLevel( baseDrawLevel + 2 )
				c:SetFont( "$(BOLD_FONT)|$(KB_18)|soft-shadow-thick" )
				c:SetAnchor( CENTER )
...
--[[
		local numCats = GetNumFurnitureCategories()
		for index =  do
			ui.BuildTemplate:AddItem( templateName, EHT.UI.BuildParamsChanged )
	end
	ui.BuildTemplate:SetSelectedItem( EHT.SavedVars.Build.TemplateName )
...
---[ Placeable Furniture Filters ]---

do
	local originalGetPlaceableFurnitureCache
	local placeableFurnitureCache
	local filteredPlaceableFurnitureCache
	local validBagIds
	local boundFilter

	function EHT.UI.GetPlaceableFurnitureBagIds()
		return validBagIds
	end

	function EHT.UI.RefreshPlaceableFurnitureList()
		SHARED_FURNITURE:FireCallbacks("PlaceableFurnitureChanged")
		SHARED_FURNITURE:RequestApplyPlaceableTextFilterToData()
	end

	function EHT.UI.SetPlaceableFurnitureBagIds( bagIds )
		if "table" ~= type( bagIds ) or 0 == #bagIds then
			validBagIds = nil
		else
			validBagIds = { }
			for index, bagId in ipairs( bagIds ) do
				validBagIds[ bagId ] = true
			end
		end

		EHT.UI.RefreshPlaceableFurnitureList()
	end

	function EHT.UI.ClearPlaceableFurnitureFilter()
		EHT.UI.SetPlaceableFurnitureBagIds()
	end

	function EHT.UI.FilterPlaceableFurnitureByBackpack()
		EHT.UI.SetPlaceableFurnitureBagIds( { BAG_BACKPACK } )
	end

	function EHT.UI.FilterPlaceableFurnitureByBank()
		EHT.UI.SetPlaceableFurnitureBagIds( { BAG_BANK, BAG_SUBSCRIBER_BANK } )
	end

	function EHT.UI.FilterPlaceableFurnitureByHouseBanks()
		EHT.UI.SetPlaceableFurnitureBagIds( { BAG_HOUSE_BANK_ONE, BAG_HOUSE_BANK_TWO, BAG_HOUSE_BANK_THREE, BAG_HOUSE_BANK_FOUR, BAG_HOUSE_BANK_FIVE, BAG_HOUSE_BANK_SIX, BAG_HOUSE_BANK_SEVEN, BAG_HOUSE_BANK_EIGHT, BAG_HOUSE_BANK_NINE, BAG_HOUSE_BANK_TEN } )
	end
	
	function EHT.UI.ClearBoundFilterPlaceableFurniture()
		boundFilter = nil
		EHT.UI.RefreshPlaceableFurnitureList()
	end

	function EHT.UI.FilterPlaceableFurnitureByBound()
		boundFilter = true
		EHT.UI.RefreshPlaceableFurnitureList()
	end

	function EHT.UI.FilterPlaceableFurnitureByUnbound()
		boundFilter = false
		EHT.UI.RefreshPlaceableFurnitureList()
	end

	function EHT.UI.SetupFilterPlaceableFurniture()
		if originalGetPlaceableFurnitureCache or not SHARED_FURNITURE or not SHARED_FURNITURE.GetPlaceableFurnitureCache then
			return
		end
		originalGetPlaceableFurnitureCache = SHARED_FURNITURE.GetPlaceableFurnitureCache

		SHARED_FURNITURE.GetPlaceableFurnitureCache = function( self, itemType, ... )
			local cache = originalGetPlaceableFurnitureCache( self, itemType, ... )
			if not cache or itemType == ZO_PLACEABLE_TYPE_COLLECTIBLE then
				return cache
			end

			placeableFurnitureCache = cache
			filteredPlaceableFurnitureCache = { }

			for bagId, cachedItems in pairs( placeableFurnitureCache ) do
				if not validBagIds or validBagIds[ bagId ] then
					if boundFilter ~= nil then
						for slotIndex, cacheItem in pairs( cachedItems ) do
							if cacheItem.passesTextFilter then
								cacheItem.passesTextFilter = IsItemBound( bagId, slotIndex ) == boundFilter
							end
						end
					end
					filteredPlaceableFurnitureCache[ bagId ] = cachedItems
				end
			end

			return filteredPlaceableFurnitureCache
		end
	end
end

---[ Retrieval Furniture Filters ]---

do
	local originalGetRetrievableFurnitureCache
	local boundFilter

	function EHT.UI.RefreshRetrievableFurnitureList()
		SHARED_FURNITURE:FireCallbacks("RetrievableFurnitureChanged")
		SHARED_FURNITURE:RequestApplyRetrievableTextFilterToData()
	end

	function EHT.UI.ClearBoundFilterRetrievableFurniture()
		boundFilter = nil
		EHT.UI.RefreshRetrievableFurnitureList()
	end

	function EHT.UI.FilterRetrievableFurnitureByBound()
		boundFilter = true
		EHT.UI.RefreshRetrievableFurnitureList()
	end

	function EHT.UI.FilterRetrievableFurnitureByUnbound()
		boundFilter = false
		EHT.UI.RefreshRetrievableFurnitureList()
	end

	function EHT.UI.SetupFilterRetrievableFurniture()
		if originalGetRetrievableFurnitureCache or not SHARED_FURNITURE or not SHARED_FURNITURE.GetRetrievableFurnitureCache then
			return
		end
		originalGetRetrievableFurnitureCache = SHARED_FURNITURE.GetRetrievableFurnitureCache

		SHARED_FURNITURE.GetRetrievableFurnitureCache = function( self, ... )
			local cache = originalGetRetrievableFurnitureCache( self, ... )

			if cache then
				if boundFilter ~= nil then
					for _, cacheItem in pairs( cache ) do
						if cacheItem.passesTextFilter then
							local itemLink, collectibleLink = GetPlacedFurnitureLink( cacheItem.retrievableFurnitureId )
							if collectibleLink == "" then
								local isBound = IsItemLinkBound( itemLink )
								cacheItem.passesTextFilter = isBound == boundFilter
							end
						end
					end
				end
...
function EHT.UI.ShowNewInstallationDialog()
	...
	if nil == ui then
		...
		b = WINDOW_MANAGER:CreateControl( nil, win, CT_CONTROL )
		ui.Body = b
...
do
	...
	function DirectionalIndicators:Initialize( ... )
		...
		for index = 1, 2 do
			...
			local arrow = WINDOW_MANAGER:CreateControl( nil, window, CT_TEXTURE )
...
do
	local lastIdentification = 0

	local function OnFurnitureIdentified( id )
		if not id or 0 == id then
			EHT.UI.DisplayNotification( "Target a furnishing and try again." )
			local FORCE = true
			EHT.UI.HideItemInfoDialog( FORCE )
		else
			local SUPPRESS_AUTO_HIDE = true
			EHT.UI.ShowItemInfoDialog( id, nil, SUPPRESS_AUTO_HIDE )
		end
	end

	function EHT.UI.ShowTargetFurnitureInfo()
		local ft = GetFrameTimeMilliseconds()
		if 250 > ft - lastIdentification then
			return
		end
		lastIdentification = ft
		EHT.Housing.IdentifyTargetFurnitureAndNode( OnFurnitureIdentified )
	end
end
...
do
	...
	function HUDFragment:Initialize()
		...
		ZO_KeybindButtonTemplate_Setup( button, "EHT_IDENTIFY_FURNISHING", function( ... ) self:OnIdentifyFurnishing( ... ) end, "Identify" )
		button:SetExcludeFromResizeToFitExtents( true )
	...
	function HUDFragment:IsDisabled()
		return GetCurrentZoneHouseId() == 0
	end

	function HUDFragment:UpdateVisibility()
		zo_callLater( function()
			local hidden = self:IsDisabled() or self:IsHidden()
			self.control:SetHidden( hidden )
		end, 500 )
	end

	function HUDFragment:OnIdentifyFurnishing()
		if not self:IsDisabled() then
			EHT.UI.ShowTargetFurnitureInfo()
		end
	end

	EHT.HUDFragment = HUDFragment:New()
end
