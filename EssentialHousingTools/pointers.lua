	...
	if nil ~= pointers then
		...
		for index = 1, #pointers.Pointers do
			...
			if not p:IsHidden() then
				...
				--if index == EHT.CONST.POINTER.SELECTED then
					--local coeff = 1 + 2 * math.abs( -1 + 2 * ( ( GetFrameTimeMilliseconds() % 4000 ) / 4000 ) )
					--p:Set3DLocalDimensions( coeff * p.LocalWidth, coeff * p.LocalHeight )
				--end

				--if p.Rotates then
					--pitch, _, roll = p:Get3DRenderSpaceOrientation()
					--p:Set3DRenderSpaceOrientation( pitch, globalYaw, roll )
				--end
...
function EHT.Pointers.OnGroupOutlineUpdate()
	...
	if ui then
		...
		local ft = GetFrameTimeMilliseconds()
		local coords1 = EHT.World:GetLinearInterval( 22000 )
		local coords2 = EHT.World:GetLinearInterval( 6000 )
		local cl = EHT.World:GetLoopInterval( 8000 )
		local ca1 = globalAlpha * math.min( 1, ( -0.5 + 2 * cl ) )
		local ca2 = globalAlpha - ca1
		local r1, g1, b1 = zo_lerp( 1, 0, cl ), zo_lerp( 1, 0.75, cl ), zo_lerp( 0, 1, cl )
		local r2, g2, b2 = zo_lerp( 1, 0, cl ), zo_lerp( 0.75, 1, cl ), zo_lerp( 0, 1, cl )
		...
		for index = 1, numIndicators do
			...
			if indicator and not indicator:IsHidden() then
				...
				local l = 0.5 + 0.5 * EHT.World:DotProduct( fx, fy, fz, n[1], n[2], n[3] )
				...
				if 2 >= index then
					indicator:SetColor( fr, fg, fb, ca2 )
				elseif 4 >= index then
					indicator:SetColor( fr, fg, fb, ca1 )
				else
					indicator:SetVertexColors( 3, fr, fg, fb, ca1 )
					indicator:SetVertexColors( 12, fr, fg, fb, ca2 )
				end
				if 1 == ( index % 2 ) then
