...
--	EHT.UI.RegisterHousingHubScene()

	EHT.Data.ClearHouseLookupCache()
	EHT.Handlers.ResetFurnitureChangeSuppression()
	EHT.UI.SetupFilterPlaceableFurniture()
	EHT.UI.SetupFilterRetrievableFurniture()
	...
	if zoneChanged and EHT.CurrentHousePopulation > 1 then
		EHT.UI.OnHouseGuestsPresent( EHT.CurrentHousePopulation )
	end

	EHT.TriggerPhrases = { }
	EHT.TriggerQueue = { }
	EHT.Housing.CacheAllInitialFurnitureStates()
	EHT.Data.RefreshTriggerStates()
	EHT.Biz.QueueQueryStatefulFurniture()
	...
	--EVENT_MANAGER:RegisterForUpdate( "EHT.Housing.ValidateOpenHousePermissions", 15000, EHT.Housing.ValidateOpenHousePermissions )
...
function EHT.Handlers.OnFurnitureStateChanged( id, newState, oldState )
	EHT.Handlers.OnRecordingSceneFurnitureStateChanged( id, newState, oldState )
	EHT.EffectFurnitureStateManager:OnFurnitureStateChanged( id, newState, oldState )

	local triggers = EHT.Biz.MatchTriggersByFurnitureState( id, newState )
	if triggers and 0 ~= #triggers then
		EHT.Biz.QueueTriggerActions( triggers, EHT.CONST.TRIGGER_CONDITION.FURNITURE_STATE )
	end
--[[
	local key = string.fromId64(key)
	if key then
		EHT.FurnitureStates[key] = newState
	end
]]
end
...
--[[
	if EHT.GetSetting( "EnableEasySlide" ) then
		if newMode == HOUSING_EDITOR_MODE_PLACEMENT then
			EVENT_MANAGER:RegisterForUpdate( "EHTEditorPrecisionEdit", 400, EHT.UI.OnEditorPrecisionEdit )
		end

		if not EHT.SuppressEasySlideHint then
			if newMode == HOUSING_EDITOR_MODE_SELECTION then
				EHT.UI.ShowHint(
					"|c00ffffSingle Click|r to pick an item up\n|c00ffffClick and Hold|r to slide an item along any axis",
					5000,
					function()
						EHT.SuppressEasySlideHint = true
					end
				)
			else
				EHT.UI.HideHint()
	...
	if nil == furnitureId then
		...
		EHT.OriginTabTargetId = nil
		EHT.AdjacentTabTargetIds = nil
	end

	if not EHT.TabTargeting then
	...
	-- Always clear Tab Target flag when switching back to Selection mode from Placement mode.
	if newMode == HOUSING_EDITOR_MODE_PLACEMENT and oldMode == HOUSING_EDITOR_MODE_SELECTION then
		EHT.TabTargeting = false
	end
end
...
function EHT.Handlers.OnFurniturePlaced(_, furnitureId, collectibleId, suppressChangeTracking)
	...
	if nil ~= furnitureId and 0 ~= furnitureId then
		...
		EHT.Housing.CacheInitialFurnitureState(furnitureId)
...
function EHT.Handlers.OnFurnitureRemoved(event, furnitureId, collectibleId, furnitureRecord)
	...
	if nil ~= furnitureId and 0 ~= furnitureId then
		...
		EHT.Housing.ClearCachedFurnitureState(furnitureId)
...
function EHT.Handlers.OnChatMessage( evt, channel, from, text, isCustServ, fromDN )
	...
	if not text or not channel or not EHT.Biz or not EHT.Util or not EHT.Util.Trim or not EHT.CONST or not EHT.CONST.TRIGGER_CONDITION then return end

	text = EHT.Util.Trim( text )
	if "" == text then return end

	if EHT.TriggerPhrases and not EHT.Biz.AreTriggersDisabled() then
		if channel == CHAT_CHANNEL_SAY or channel == CHAT_CHANNEL_PARTY or channel == CHAT_CHANNEL_WHISPER or channel == CHAT_CHANNEL_WHISPER_SENT or channel == CHAT_CHANNEL_ZONE or channel == CHAT_CHANNEL_YELL then
			...
			if triggers then EHT.Biz.QueueTriggerActions( triggers, EHT.CONST.TRIGGER_CONDITION.PHRASE ) end
		end
	end

	EHT.UI.ParseHouseLinks( text )
end
	...
	if channel then
		...
		if nil ~= fromDN and "" ~= fromDN then
			if string.sub( text or "", 1, #EHT.CHATCAST_PREFIX ) == EHT.CHATCAST_PREFIX then
				...
				EHT.Effect:ProcessInboundChatcast( fromDN, text, "Chat" ) -- string.sub( text, ( #EHT.CHATCAST_PREFIX + 1 ) ) )
				...
				if ownerEndIndex then
					...
					if houseNameEndIndex then
						...
						if houseData then
							...
							-- Replace this tag with an interactive link to jump to the house.
							local textStart = 1 < tagStartIndex and string.sub( text, 1, tagStartIndex - 1 ) or ""
							local textEnd = houseNameEndIndex < #text and string.sub( text, houseNameEndIndex + 1 ) or ""
							text = textStart .. EHT.UI.CreateHouseLink( ownerName, houseId ) .. textEnd
							return text, nil -- info.saveTarget
						end
					end
				end
			end
...
--[[
	local chatEventHandlers = CHAT_ROUTER.registeredEventHandlers
	if not chatEventHandlers then
		chatEventHandlers = ZO_ChatSystem_GetEventHandlers()
	end

	if chatEventHandlers then
		if EHT.Handlers.OnChatFilter ~= chatEventHandlers[ EVENT_CHAT_MESSAGE_CHANNEL ] then
			originalChatMessageHandler = chatEventHandlers[ EVENT_CHAT_MESSAGE_CHANNEL ]
			chatEventHandlers[ EVENT_CHAT_MESSAGE_CHANNEL ] = EHT.Handlers.OnChatFilter
		end
	end
]]
end
...
--[[
function EHT.Handlers.OnSpamWarning( eventCode, spamType )

end

function EHT.Handlers.OnGetGameCameraInteractableActionInfo( origFunc )

	local action, name, interactBlocked, isOwned, additionalInfo, contextualInfo, contextualLink, isCriminalInteract = origFunc()

	if GGC then
		return unpack( GGC ) -- "Action", "Name", false, true, nil, nil, nil, false
	else
		return action, name, interactBlocked, isOwned, additionalInfo, contextualInfo, contextualLink, isCriminalInteract
	end
...
function EHT.Handlers.OnLinkClicked( rawLinkString, mouseButton, linkText, linkStyle, linkType, ownerName, houseId )
	linkType = string.lower( tostring( linkType ) or "" )

	if "house" == linkType then
		local ownerName = tostring( ownerName ) or ""
		local houseId = tonumber( houseId ) or 0
		EHT.UI.OnHouseLinkClicked( ownerName, houseId )
		return true
	end
end
...
function EHT.Handlers.RegisterHandlers()
	if not handlersRegistered then
		...
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_SPAM_WARNING, EHT.Handlers.OnSpamWarning )
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_GUI_HIDDEN, EHT.Handlers.OnGuiHidden )
		...
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_PLAYER_STUNNED_STATE_CHANGED, EHT.Handlers.OnStunStateChanged )
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_GROUP_MEMBER_JOINED, EHT.Handlers.OnGroupMemberJoined )
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_GROUP_ELECTION_NOTIFICATION_ADDED, EHT.Handlers.OnGroupElectionNotificationAdded )
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_STEALTH_STATE_CHANGED, EHT.Handlers.OnStealthStateChanged )
		--EVENT_MANAGER:RegisterForEvent( EHT.ADDON_NAME, EVENT_GAME_CAMERA_UI_MODE_CHANGED, EHT.Handlers.OnUIModeChanged )

		ITEM_PREVIEW_KEYBOARD.fragment:RegisterCallback( "StateChange", EHT.Handlers.OnItemPreviewStateChange )
		--LINK_HANDLER:RegisterCallback( LINK_HANDLER.LINK_CLICKED_EVENT, EHT.Handlers.OnLinkClicked )
		LINK_HANDLER:RegisterCallback( LINK_HANDLER.LINK_MOUSE_UP_EVENT, EHT.Handlers.OnLinkClicked )
		...
		if not shadowsRegistered then
			...
			--EHT.Util.ShadowFunction( _G, "ToggleShowIngameGui", EHT.Handlers.OnToggleShowIngameGui )
			--EHT.Util.ShadowFunction( _G, "GetGameCameraInteractableActionInfo", EHT.Handlers.OnGetGameCameraInteractableActionInfo )
...
function EHT.Handlers.UnregisterHandlers()
	if handlersRegistered then
		...
		--EVENT_MANAGER:UnregisterForEvent( EHT.ADDON_NAME, EVENT_SCREENSHOT_SAVED, EHT.Handlers.OnScreenshotSaved )
		...
		--LINK_HANDLER:UnregisterCallback( LINK_HANDLER.LINK_CLICKED_EVENT, EHT.Handlers.OnLinkClicked )
		LINK_HANDLER:UnregisterCallback( LINK_HANDLER.LINK_MOUSE_UP_EVENT, EHT.Handlers.OnLinkClicked )
