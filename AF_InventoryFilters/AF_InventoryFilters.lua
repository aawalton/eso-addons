local name = "AF_InventoryFilters"
...
local function OnPlayerActivated( eventCode, initial )
	...
	local GetFilterCallbackForInventoryFilters = function( account )
	...
	local filterInformation = {
		callbackTable = {
			{ name = "InventoryFilters", filterCallback = GetFilterCallbackForInventoryFilters() },
		},

		enStrings = {
			["InventoryFilters"] = "Uncollected",
			["AnyAccount"] = "(Any)",
		},

		deStrings = {
			["InventoryFilters"] = "Nicht gesammelt",
	...
	if (LMAS) then
		...
		if (#otherAccounts > 0) then
			-- Set up submenu
			filterInformation.submenuName = "InventoryFilters"
			...
			table.insert(filterInformation.callbackTable, {
				name = "AnyAccount",
				filterCallback = GetFilterCallbackForInventoryFilters("Any"),
			})
			...
			for _, account in ipairs(otherAccounts) do
				table.insert(filterInformation.callbackTable, {
					name = account,
					filterCallback = GetFilterCallbackForInventoryFilters(account),
				})
