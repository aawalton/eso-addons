...
function LootLogTrade.HookLists( )
	...
	EVENT_MANAGER:RegisterForEvent(LootLogTrade.name, EVENT_OPEN_TRADING_HOUSE, function( eventCode )
		EVENT_MANAGER:UnregisterForEvent(LootLogTrade.name, EVENT_OPEN_TRADING_HOUSE)
		ProcessListHooks({
...
function LootLogTrade.LinkTrade( ... )
	...
	for _, item in pairs(bagCache) do
		...
		if (not IsItemPlayerLocked(item.bagId, item.slotIndex) and not IsItemLinkBound(itemLink) and filter(item) and IsItemLinkSetCollectionPiece(itemLink)) then
