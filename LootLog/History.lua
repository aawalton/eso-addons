...
function LootLogList:Setup( )
	...
	self:InitializeComboBox(self.filterDrop)
...
function LootLogList:InitializeComboBox( control )
	...
	for i = 1, 6 do
		local entry = ZO_ComboBox:CreateItemEntry(GetString("SI_LOOTLOG_MODE", i), callback)
		entry.id = i
		control:AddItem(entry, ZO_COMBOBOX_SUPRESS_UPDATE)
	end

	LEJ.SelectComboBoxItemByIndex(control, LootLog.vars.historyMode, true)
end
