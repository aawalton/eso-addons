...
function WritWorthy.InitAGSIntegration(trading_house_wrapper)
    ...
    function WW_AGS_Filter:FilterLocalResult(item_data)
        ...
        local can_craft = WritWorthy.CanCraft(item_link)
        if (not can_craft) then return false end
...
function WritWorthy.CanCraft(item_link)
    local mat_list, know_list = WritWorthy.ToMatKnowList(item_link)
    local mat_text = WritWorthy.MatHaveCtTooltipText(mat_list)
    if mat_text ~= '' then return false end
    local know_text = WritWorthy.KnowTooltipText(know_list)
    if know_text ~= '' then return false end
    return true
end