...
do
	local orgSetHidden = ZO_PlayerInventoryMenuBarLabel.SetHidden
	local ZO_PlayerInventorySearchBox = PLAYER_INVENTORY.inventories[INVENTORY_BACKPACK].searchBox
	local ZO_CraftBagSearchBox = PLAYER_INVENTORY.inventories[INVENTORY_CRAFT_BAG].searchBox
	function ZO_PlayerInventoryMenuBarLabel:SetHidden(hide)
		return orgSetHidden(self, hide or ZO_PlayerInventorySearchBox:GetText() ~= "" or ZO_CraftBagSearchBox:GetText() ~= "")
	end
end
...
local function SetupSearchBox(bagSearchBg, ...)
	...
	function bagSearch.SetText(...)
		if wm:GetFocusControl() ~= bagSearch then
            ...
            cursorPosition = bagSearch:GetCursorPosition()
    		orgSetText(...)
            bagSearch:SetCursorPosition(cursorPosition)
		end
	end
...
local function AddQuickSlotFilter(searchBox)
	...
	local orgShouldAddQuestItemToList = ZO_QuickslotManager.ShouldAddQuestItemToList
	...
	local orgAppendQuestItemData = ZO_QuickslotManager.AppendQuestItemData
	function ZO_QuickslotManager.AppendQuestItemData(...)
...
local function SetupControls()
	...
	SetupInventoryControl(
		INVENTORY_BACKPACK,
		"CraftBagExtendedBankMenu",
		"CraftBagExtendedGuildBankMenu",
		"CraftBagExtendedVendorMenu",
		"ZO_StoreWindowMenuBar_Auto Recharge_VENDOR_REPAIRKITS",
		"ZO_StoreWindowMenuBar_Auto Recharge_VENDOR_REPAIRKITS_LABEL"
	)
	...
	local searchBox = QUICKSLOT_WINDOW.searchBox
	local bagSearch = searchBox:GetParent()
	local parent = bagSearch:GetParent()
	parent:SetResizeToFitDescendents(false)
	bagSearch:SetParent(parent:GetParent())
	HookShouldAddItemToListSearchBox(bagSearch, ZO_QuickSlot, QUICKSLOT_WINDOW)
